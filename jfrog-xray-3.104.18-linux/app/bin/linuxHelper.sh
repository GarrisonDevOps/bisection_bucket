...
POSTGRES_MAJOR_VER="15"
POSTGRES_SLES_MAJOR_VER="15"
POSTGRES_SLES_VER="15.6-1"
POSTGRES_RPM_VER="15.6-1"
POSTGRES_DEB_VER="15_15.6-1"
...
setPostgresVersion() {
    ...
    JFROG_POSTGRES_15_2X_VERSION="15-2v"
    JFROG_POSTGRES_15_6X_VERSION="15-6v"
    ...
    case "$PRODUCT_NAME" in
        ...
        $XRAY_LABEL)
            ...
            JFROG_POSTGRES_VERSIONS=("9.5.2" "10.13" "12.3" "12.5" "13.2" "13.4" "13.9" "13.10" "15.2" "15.6")
        ;;
    esac
}
...
testVarPermissions () {
    local srcProductData="${JF_PRODUCT_HOME}/var"
    local targetProductData="${JF_PRODUCT_HOME}/var"

    if [ -L "${srcProductData}" ]; then
        # Handle darwin and linux - option -f does not work on darwin
        targetProductData=$(readlink -f ${srcProductData} 2>/dev/null || readlink ${srcProductData} 2>/dev/null)
        [ ! -z "${targetProductData}" ] || errorExit "Cound not read link from ${srcProductData}, please make sure ${srcProductData} is a valid link (will be used to store configuration and data)"
    fi

    mkdir -p "${targetProductData}" || errorExit "Unable to create directory : ${targetProductData}"
    testDirectoryPermissions "${targetProductData}"
}

createUserAndGroup(){
    [[ -z "${PRODUCT_USER}" && -z "${DEFAULT_PRODUCT_USER}" ]] \
        && errorExit "PRODUCT_USER is needed to continue with installation, set this as an evironment variable to continue" || true

    : ${PRODUCT_USER:="${DEFAULT_PRODUCT_USER}"}
    : ${PRODUCT_GROUP:="${PRODUCT_USER}"}

    getent group "${PRODUCT_GROUP}" 1>/dev/null 2>&1
    if [ $? != 0  ]; then
      logger "Group ${PRODUCT_GROUP} doesn't exist. Creating ..."
      /usr/sbin/groupadd -r ${PRODUCT_GROUP} 1>/dev/null 2>&1 \
          || errorExit "Could not add group, command [ /usr/sbin/groupadd -r ${PRODUCT_GROUP} ]"
    else
      logger "Group $PRODUCT_GROUP already exists"
    fi

    getent passwd "${PRODUCT_USER}" 1>/dev/null 2>&1
    if [ $? != 0 ]; then
      logger "User $PRODUCT_USER doesn't exist. Creating ..."
      /usr/sbin/useradd -r -d "${JF_PRODUCT_HOME}" -s /bin/bash -m -g "${PRODUCT_GROUP}" "${PRODUCT_USER}" 1>/dev/null 2>&1 \
          || errorExit "Could not add group, command [ /usr/sbin/useradd -r -d ${JF_PRODUCT_HOME} -s /bin/bash -m -g ${PRODUCT_GROUP} ${PRODUCT_USER} ]"
      chown ${PRODUCT_USER}:${PRODUCT_USER} "${JF_PRODUCT_HOME}"
    else
      logger "User $PRODUCT_USER already exists"
    fi
}

# Any common actions to be performed at the beggining of a startup script can be maintained here
# Can be enhanced over time by adding more types and validations
# This should have only methods which does not need arguments to be passed
commonStartupActions(){
    local type="${1:-docker}"
    if [ "${type}" == "docker" ]; then
        validateDiskSpace
    fi
...
setUserGroup(){
    ...
    local groupKey="${2}"
    local defaultUser="${3}"
    local defaultGroup="${4:-"${defaultUser}"}"
    local user=
    local group=

    [[ ! -z "${userKey}"      ]] || return 0;
    [[ ! -z "${groupKey}"     ]] || return 0;
    [[ ! -z "${defaultUser}"  ]] || return 0;
    [[ ! -z "${defaultGroup}" ]] || return 0;
    ...
    # Default it to default user
    PRODUCT_USER=${user:-${defaultUser}}
    PRODUCT_GROUP=${group:-${defaultGroup}}
}

# Check if the script is running with the same user specified in system.yaml
# This can be used in scripts where you want to be sure that the current script is running with the intended user
checkUser(){
    local ymlKey="${1:-shared.user}"
    local exitOnError="${2:-${FLAG_Y}}"
    local currentUser="$USER"
    local expectedUser=

    [ ! -z "${ymlKey}" ] || return 0

    getSystemValue "${ymlKey}" "NOT_SET" "false" "${JF_SYSTEM_YAML}"
    expectedUser="${YAML_VALUE}"

    if [[ -z "${expectedUser}" || "${expectedUser}" == "NOT_SET" || -z "${currentUser}" ]]; then
        return 0;
    fi

    local errorMessage=$(cat << ERROR_MESSAGE
Found ${ymlKey} set as ${expectedUser} in ${JF_SYSTEM_YAML}, this script is started as ${currentUser}. 
This script can only be invoked by ${expectedUser} as set in ${JF_SYSTEM_YAML}.
ERROR_MESSAGE
)

    if [[ "${currentUser}" != *${expectedUser}* ]]; then
        [[ "${exitOnError}" == "${FLAG_Y}" ]] && errorExit "${errorMessage}" || return 1;
    fi
}
...
checkAndSetOwnerOnDir () {
    local dirToCheck=$1
    local userToCheck=$2
    local groupToCheck=$3

    if [[ -z "${dirToCheck}" || -z "${userToCheck}" || -z "${groupToCheck}" ]]; then
        warn "A user, group and a directory is mandatory to check ownership"
        return
    fi

    if [ ! -d "${dirToCheck}" ]; then
        warn "${dirToCheck} is not a directory, skipping setting of ownership"
        return
    fi

    logger "Checking permissions on $dirToCheck"
    local STAT=( $(stat -Lc "%U %G" ${dirToCheck}) )
    local USER=${STAT[0]}
    local GROUP=${STAT[1]}

    if [[ "${USER}" != "$userToCheck" ]] || [[ "${GROUP}" != "$groupToCheck"  ]] ; then
        logger "$dirToCheck is owned by $USER:$GROUP. Setting to $userToCheck:$groupToCheck."
        chown -R "${userToCheck}":"${groupToCheck}" "${dirToCheck}" || errorExit "Setting ownership on $dirToCheck failed"
    else
        logger "$dirToCheck is already owned by $userToCheck:$groupToCheck."
    fi
}

setRpmDebDataOwnership(){
    local user=$1
    local group=${2:-$1}
    ...
    local dataLink="${JF_PRODUCT_HOME}/var"
    local dataDir=$(readlink -f "${dataLink}" 2>/dev/null)

    if [ -L "${dataLink}" ]; then
      chown -h "${user}":"${group}" "${dataLink}" >&6 2>&1 \
          || errorExit "Failed to set ownership (${user}:${group}) on ${dataLink}"
    fi

    if [ -d "${dataDir}" ]; then
      io_setOwnership "${dataDir}" "${user}" "${group}" >&6 2>&1 \
          || errorExit "Failed to set ownership (${user}:${group}) on ${dataDir}"
    fi
}

changeRpmDebOwnership(){
    local productHome=${1:-${JF_PRODUCT_HOME}}
    local user=${2}
    local group=${3:-${user}}

    if [ -z "${user}" ]  || [ -z "${group}" ] || [ -z "${productHome}" ]; then
        warn "A JF_PRODUCT_HOME, user and group is mandatory to set ownership on JF_PRODUCT_HOME/app"
    fi

    createDir "${JF_PRODUCT_HOME}/var/log" "${user}" "${group}"

    io_setOwnership  "${JF_PRODUCT_HOME}/app" "${user}" "${group}" || errorExit "Setting ownership of [${JF_PRODUCT_HOME}/app] to [${user}:${group}] failed"

    if [ -d "${JF_PRODUCT_HOME}/var/etc" ]; then
        io_setOwnership  "${JF_PRODUCT_HOME}/var/etc" "${user}" "${group}" || errorExit "Setting ownership of [${JF_PRODUCT_HOME}/app] to [${user}:${group}] failed"
    fi

    if [ -d "${JF_PRODUCT_HOME}/var/bootstrap" ]; then
        io_setOwnership  "${JF_PRODUCT_HOME}/var/bootstrap" "${user}" "${group}" \
            || errorExit "Setting ownership of [${JF_PRODUCT_HOME}/var/bootstrap] to [${user}:${group}] failed"
    fi
}
...
checkOwnerAndExit(){
    ...
    if [ -z "${mainDir}" ] || [ -z "${usr}" ] || [ -z "${grp}" ]; then
        warn "A directory (${mainDir}), user (${usr}) and group (${grp}) are manadtory parameters to modify ownership"
        return 0
    fi

    [ -d "${mainDir}" ] || [ -L "${mainDir}" ] || [ -f "${mainDir}" ] || return 0

    io_checkOwner "${mainDir}" "${usr}" "${grp}" "${checkName}"
    if [ $? == 1 ] ; then
        logger "##############################################################################" "ERROR"
        logger "Ownership mismatch. You can try executing following instruction and do a restart" "ERROR"
        logger "Command : chown -R ${usr}:${grp} ${mainDir}" "ERROR"
        logger "##############################################################################" "ERROR"
        exit 1
    fi
}

# Validate given directory and its first level of children are set with provided ownership
testOwnership(){
    local mainDir=${1:-${JF_PRODUCT_HOME}/var}
    local usr=${2}
    local grp=${3:-${usr}}

    if [ -z "${mainDir}" ] || [ -z "${usr}" ] || [ -z "${grp}" ]; then
        warn "A directory (${mainDir}), user (${usr}) and group (${grp}) are mandatory parameters to modify ownership"
        return 0
    fi

    [ -d "${mainDir}" ] || [ -L "${mainDir}" ] || [ -f "${mainDir}" ] || return 0

    checkOwnerAndExit "${mainDir}" "${usr}" "${grp}"

    if ls ${mainDir}/ >/dev/null 2>&1 ; then
        local dirContent=${mainDir}/*
        for item in ${dirContent}
        do
            # Skip this item if its niether directory nor link nor a file
            [ -d "${item}" ] || [ -f "${item}" ] || [ -L "${item}" ] || continue

            checkOwnerAndExit "${item}" "${usr}" "${grp}"
        done
    fi
}
...
setVarLinkOwnership(){
    ...
    if [[ "${curTargetVar}" == *"${varLink}"* ]]; then
        ...
        mkdir -p "${newTargetVar}"                                   || errorExit "Create directory failed, command : mkdir -p \"${newTargetVar}\""
        checkAndSetOwnerOnDir "${newTargetVar}" "${user}" "${group}"
        ...
        mkdir -p "${targetVar}"                || errorExit "Create directory failed, command : mkdir -p \"${targetVar}\""
        checkAndSetOwnerOnDir "${targetVar}" "${user}" "${group}"
...
# Take a key and default as input,
#       Throw an log message if property is set in system.yaml
#       Set equivalent environment variable with the default value
setNodeProperty(){
    local key=$1
    ...
    local envValue=
    local logDefaultMsg="false"

    [ ! -z "${key}" ] || return

    transformedKey=$(transformKeyToEnv $key)
    envValue="${!transformedKey}"

    getSystemValue "${key}" "NOT_SET" "$logDefaultMsg"
    value="${YAML_VALUE}"

    if [[ "${value}" == "NOT_SET" ]]; then
        value="$defaultValue"
        logger "Setting $transformedKey to $value"
        eval $transformedKey="${value}"
        [ "${key}" == "${SYS_KEY_SHARED_NODE_IP}" ] &&  modifyValue "$transformedKey" "HOST_IP" || true
        export $transformedKey
    elif [[ -z "${envValue}" && "${value}" != "NOT_SET" ]]; then
        logger "Found explicit ${key} in system.yaml with value : $value"
        eval $transformedKey="${value}"
        [ "${key}" == "${SYS_KEY_SHARED_NODE_IP}" ] &&  modifyValue "$transformedKey" "HOST_IP" || true
        export $transformedKey
    elif [[ ! -z "${envValue}" ]]; then
        logger "Found $transformedKey set as environment variable with value : $envValue"
        [ "${key}" == "${SYS_KEY_SHARED_NODE_IP}" ] &&  modifyValue "$transformedKey" "HOST_IP" || true
        export $transformedKey
    fi
}
...
setupNodeDetails () {
    setNodeProperty "${SYS_KEY_SHARED_NODE_ID}" "$(io_getPublicHostID)"
    setNodeProperty "${SYS_KEY_SHARED_NODE_IP}" "$(io_getPublicHostIP)"
    setNodeProperty "${SYS_KEY_SHARED_NODE_NAME}" "$(io_getPublicHostName)"
}

# Translate a value from given list of old keys to new keys
# Expects a argument with list of env mapping.
# Example ENV_MAP="""shared.node.id=HA_NODE_ID
#                    connector.extra=SERVER_XML_EXTRA_CONNECTOR"""
translateEnv() {
    local envMap="$1"
    local oldKey=
    local newKey=
    ...
    do
        [[ $entry == *"="* ]] || continue;
        newKey=$(echo "${entry}" | awk -F'=' '{print $1}')
        oldKey=$(echo "${entry}" | awk -F'=' '{print $2}')

        [ ! -z "$newKey" ] || continue;
        [ ! -z "$oldKey" ] || continue;

        # Skip translation of this variable if new key is already set with a value 
        getSystemValue "${newKey}" "NOT_SET"
        newValue="${YAML_VALUE}"

        # Proceed with translation only if new key is not set - new key takes priority over old key
        if [[ "$newValue" != "NOT_SET" ]]; then 
            continue
        fi

        # allowed oldValues in a environment key are upper case alphabet, number and underscore 
        [[ "$oldKey" =~ ^[A-Z_0-9]+$ ]] || continue;

        oldValue="${!oldKey}"

        if [ ! -z "${oldValue}" ]; then
            envEquivalent=$(transformKeyToEnv "${newKey}")

            logger "Mapping value from deprecated key ${oldKey} to new key ${envEquivalent}"
            export ${envEquivalent}="${oldValue}"
        fi
    done
}

# Load environment variable specified for a given service from system.yaml
# Given a service name it will append env to get and load all the env variables in it,
# For instance, if metadata is passed, metadata.env will be loaded
#
# Expects a service name as parameter,
# Sample usage :
#       exportEnv shared
#       exportEnv metadata
exportEnv(){
    ...
    envPath="${service}.env"
    getSystemValue "${envPath}" "NOT_SET" "false"
    envList="${YAML_VALUE}"

    [[ "${envList}" != "NOT_SET" ]] || return
    
    OLDIFS=$IFS
    # Use newline as the only delemiter, this is to avoid transformation of a value to JF format 
    #       resulting in failure of script in case of slash and other special characters
    IFS=$'\n'
    for entry in ${envList}
    do
        if [[ "${entry}" =~ ^JF_* ]]; then
            # Throw a warning if product related environment variable (JF_*) is added as part of env list
            warn "Environment variable starting with JF_ is not allowed in ${envPath}. Please use its equivalent yaml path to pass it to application."
            continue
        fi

        key=$(echo "${entry}" | awk -F"${delimiter}" '{print $1}')
        key="$(io_trim "${key}")"

        getSystemValue "${envPath}.${key}" "NOT_SET"
        value="${YAML_VALUE}"

        [[ "${value}" != "NOT_SET" ]] || continue
...
displayRpmDebStartInfo () {
    local packName="$1"
    local productName="${2:-product}"

    if [[ -z "${packName}" || -z "${productName}" ]]; then
        return 0
    fi

    local systemctlInfo=$(cat << END_START_USAGE

Start ${productName} with:
> systemctl start ${packName}.service

Check ${productName} status with:
> systemctl status ${packName}.service

END_START_USAGE
) 

    local sysvInfo=$(cat << END_START_USAGE

Start ${productName} with:
> service ${packName} start 

Check ${productName} status with:
> service ${packName} status 
END_START_USAGE
) 

    local isSystemctlAvailable=$(hash systemctl 2>/dev/null; echo $?)

    if [ "${isSystemctlAvailable}" == "0" ]; then
        echo "${systemctlInfo}"
    else
        echo "${sysvInfo}"
    fi

    cat << EXTRA_START_INFO

Installation directory was set to ${JF_PRODUCT_HOME}
You can find more information in the log directory ${JF_PRODUCT_HOME}/var/log

System configuration templates can be found under ${JF_PRODUCT_HOME}/var/etc
Copy any configuration you want to modify from the template to ${JF_PRODUCT_HOME}/var/etc/system.yaml
EXTRA_START_INFO
}

displayRpmDebStartTrialInfo () {
    local packNameRt="$1"
    local packNameXray="$2"
    local productName="$3"

    if [[ -z "${packNameRt}" ||  -z "${packNameRt}" || -z "${productName}" ]]; then
        return 0
    fi
...
Start ${productName} with:
1. systemctl start ${packNameRt}.service
2. systemctl start ${packNameXray}.service

Check ${productName} status with:
1. systemctl status ${packNameRt}.service
2. systemctl status ${packNameXray}.service
...
Start ${productName} with:
1. service ${packNameRt} start 
2. service ${packNameXray} start

Check ${productName} status with:
1. service ${packNameRt} status
2. service ${packNameXray} status 
END_START_USAGE
) 
...
You can find more ${ARTIFACTORY_LABEL} information in the log directory ${JF_PRODUCT_HOME_RT}/var/log
You can find more ${XRAY_LABEL} information in the log directory ${JF_PRODUCT_HOME_XRAY}/var/log
EXTRA_START_INFO
}
...
displayArtConnInfo() {
    ...
    cat << ARTIFACTORY_YAML_INFO

Please note it is mandatory to set ${SYS_KEY_SHARED_JFROGURL} and ${SYS_KEY_SHARED_SECURITY_JOINKEY} in the system.yaml file to complete the configuration.
Both of these are available in this page Admin > Security > Settings
You can read more about these configurations at: ${doclink}
ARTIFACTORY_YAML_INFO
}
...
rpmDebIsUpgradeAllowed(){
    local packName=$1
    local minVersion=$2
    local errMsg="${3:-"Upgrade from version VERSION is not supported"}"
    local name=
    local version=

    if [ -z "${packName}" ] || [ -z "${minVersion}" ] || [ -z "${errMsg}" ]; then
        return 0
    fi

    local arrIN=(${packName//=/ })
    packName=${arrIN[0]}

    rpmDebDetermineType || return 


    case $PACKAGE_TYPE in
        rpm)
            name="$(rpm -qa --nosignature --queryformat '%{NAME}\n' ${packName} 2>/dev/null)"
            version="$(rpm -qa --nosignature --queryformat '%{VERSION}\n' ${packName} 2>/dev/null)"
        ;;
        deb)
    ...
    if [ "$(io_compareVersions "${version}" "${minVersion}")" == "-1" ]; then
        echo
        # replace string VERSION in error message to current installed version
        errorExit "${errMsg/VERSION/${version}}" >&2
    fi
}
...
io_getLinuxDistribution() {
    ...
    # OS 9.x
    grep -q -i "release 9" /etc/redhat-release >/dev/null 2>&1 && LINUX_DISTRIBUTION_VER="9" || true
...
checkJavaVersion(){
    ...
    if [[ -n "$javaHome" ]] && [[ -x "$javaHome/bin/java" ]];  then
        echo "Found java executable in javaHome ($javaHome)"
...
addCertsToJavaKeystore () {
    local certsDir=${1:-${JF_PRODUCT_HOME}/var/etc/security/keys/trusted}
    local packagedJavaHome="${JF_PRODUCT_HOME}/app/third-party/java"
    local defaultKeyStorePass="changeit"
    local javaBin=

    [ ! -z "${certsDir}" ] || return 0
    [  -d  "${certsDir}" ] || return 0

    local certsList=$(ls -1 ${certsDir})
    local cert=
    ...
    if [ ! -z "${certsList}" ]; then
        if [ ! -z "${JAVA_HOME}" ]; then
            ...
            warn "JAVA_HOME is not set, certificates in ${certsDir} directory have to be manually imported to Java cacerts"
            return 0
        fi
    
        if [[ "${packagedJavaHome}" != "${JAVA_HOME}" ]]; then
            warn "Found custom JAVA_HOME (${JAVA_HOME}), skipping import of certificates. They are stored in ${certsDir}, this needs to be imported manually to your Java cacerts"
            return 0
        fi
            
        getSystemValue "shared.security.keyStorePassword" "${defaultKeyStorePass}"
        local storePass="${YAML_VALUE}"        

        for d in ${certsList}; do
            cert=${certsDir}/${d}

            # Skip directories
            [ -d "${cert}" ] && continue

            if [ ! -f "${javaBin}/keytool" ]; then
                warn "keytool not found in ${javaBin}, certificates in ${certsDir} directory have to be manually imported to Java cacerts"
                return 0
            fi
    
            # Check if alias already installed
            ${javaBin}/keytool -list -cacerts -storepass "${storePass}" -alias ${d} > /dev/null 2>&1
            if [ $? -ne 0 ]; then
                logger "Adding ${cert} to Java cacerts"
                ${javaBin}/keytool -importcert -trustcacerts -noprompt -cacerts -storepass "${storePass}" -file ${cert} -alias "${d}"
...
removeDuplicateJavaOpts () {
    local default_java_options=($1)
    local extra_java_options=($2)
    local k=

    for i in ${extra_java_options[@]}; do
      # Check for Memory parameters
      if [[ $i == -Xms* ]] || [[ $i == -Xmx* ]] || [[ $i == -Xss* ]]; then
          k=$(echo ${i:0:4})
      fi
      # Check for Heap parameters
      if [[ $i =~ ^-XX:(InitialRAMPercentage|MaxRAMPercentage) ]]; then
          k=${i%=*}
      fi
      ...
      if [[ -n "${k}" ]]; then
          ...
          local index=$(echo ${default_java_options[@]} | tr ' ' '\n' | awk "/${k}/ {print NR-1}")
          if [[ -n "${index}" ]]; then
              local v=${default_java_options[$index]}
              # Remove matching element from Array
              default_java_options=(${default_java_options[@]/${k}*/})
              logger "Removed duplicate java argument ${v}"
          fi
      fi
    done
    UNIQUE_DEFAULT_JAVA_OPTIONS="${default_java_options[@]}"
}
...
validateSystemYaml() {
    ...
    logDebug "Skipping system.yaml validation since template file is not up to date"
    return
...
wrapper_setInstallerDefaults() {
    ...
    if [[ "${PRODUCT_NAME}" == "${PDN_NODE_LABEL}" ]]; then
        wrapper_setSystemValue "$KEY_PDNNODE_SELFGRPCADDRESS" "$(io_getPublicHostIP):8088" "${overwrite}"
    fi

    if [[ "${PRODUCT_NAME}" == "${PDN_SERVER_LABEL}" ]]; then
        wrapper_setSystemValue "$KEY_PDNSERVER_SELFADDRESS" "$(io_getPublicHostIP):8095" "${overwrite}"
    fi
}

wrapper_setInstallerDefaultsRabbitmq() {
    if [ "$DONT_PROMPT_USE_DEFAULTS" == "$FLAG_Y" ];then
        rm "$INSTALLER_YAML"
        touch "${INSTALLER_YAML}"
    fi
...
wrapper_setPostgresVersion() {
    ...
    local systemYaml="${productHome}/etc/${FILE_NAME_SYSTEM_YAML}"
    local postgresVer=${JFROG_POSTGRES_15_6X_VERSION}
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
    ...
    if [ "$isPostgresExternalised" == "$FLAG_N" ] && [ ! -z "${isDataBaseUrlExist}" ]; then
        case "$installedPostgresVer" in
            ...
            ;;
            $JFROG_POSTGRES_13_10X_VERSION)
                postgresVer=${JFROG_POSTGRES_13_10X_VERSION}
            ;;
            $JFROG_POSTGRES_15_2X_VERSION)
                postgresVer=${JFROG_POSTGRES_15_2X_VERSION}
            ;;
        esac
        wrapper_setSystemValue "$KEY_POSTGRES_VERSION" "${postgresVer}" "${overwrite}"
    fi
...
INSTALL_POSTGRES_METHOD="installPostgres"

RPM_POSTGRES7="postgresql${POSTGRES_MAJOR_VER}-${POSTGRES_RPM_VER}PGDG.rhel7.x86_64"
RPM_POSTGRES8="postgresql${POSTGRES_MAJOR_VER}-${POSTGRES_RPM_VER}PGDG.rhel8.x86_64"
RPM_POSTGRES9="postgresql${POSTGRES_MAJOR_VER}-${POSTGRES_RPM_VER}PGDG.rhel9.x86_64"

RPM_POSTGRES_LIBS7="postgresql${POSTGRES_MAJOR_VER}-libs-${POSTGRES_RPM_VER}PGDG.rhel7.x86_64"
RPM_POSTGRES_LIBS8="postgresql${POSTGRES_MAJOR_VER}-libs-${POSTGRES_RPM_VER}PGDG.rhel8.x86_64"
RPM_POSTGRES_LIBS9="postgresql${POSTGRES_MAJOR_VER}-libs-${POSTGRES_RPM_VER}PGDG.rhel9.x86_64"

RPM_POSTGRES_SERVER7="postgresql${POSTGRES_MAJOR_VER}-server-${POSTGRES_RPM_VER}PGDG.rhel7.x86_64"
RPM_POSTGRES_SERVER8="postgresql${POSTGRES_MAJOR_VER}-server-${POSTGRES_RPM_VER}PGDG.rhel8.x86_64"
RPM_POSTGRES_SERVER9="postgresql${POSTGRES_MAJOR_VER}-server-${POSTGRES_RPM_VER}PGDG.rhel9.x86_64"

RPM_POSTGRES_CONTRIB7="postgresql${POSTGRES_MAJOR_VER}-contrib-${POSTGRES_RPM_VER}PGDG.rhel7.x86_64"
RPM_POSTGRES_CONTRIB8="postgresql${POSTGRES_MAJOR_VER}-contrib-${POSTGRES_RPM_VER}PGDG.rhel8.x86_64"
RPM_POSTGRES_CONTRIB9="postgresql${POSTGRES_MAJOR_VER}-contrib-${POSTGRES_RPM_VER}PGDG.rhel9.x86_64"

# For SLES
POSTGRES_SLES="postgresql${POSTGRES_SLES_MAJOR_VER}-${POSTGRES_SLES_VER}.sles12.x86_64"
POSTGRES_SERVER_SLES="postgresql${POSTGRES_SLES_MAJOR_VER}-server-${POSTGRES_SLES_VER}.sles12.x86_64"
POSTGRES_CONTRIB_SLES="postgresql${POSTGRES_SLES_MAJOR_VER}-contrib-${POSTGRES_SLES_VER}PGDG.sles12.x86_64"

UBUNTU18_POSTGRESQL="postgresql-${POSTGRES_DEB_VER}.pgdg18.04+1_amd64"
UBUNTU20_POSTGRESQL="postgresql-${POSTGRES_DEB_VER}.pgdg20.04+1_amd64"
UBUNTU22_POSTGRESQL="postgresql-${POSTGRES_DEB_VER}.pgdg22.04+1_amd64"
DEBIAN10_POSTGRESQL="postgresql-${POSTGRES_DEB_VER}.pgdg100+1_amd64"
DEBIAN11_POSTGRESQL="postgresql-${POSTGRES_DEB_VER}.pgdg110+2_amd64"

RPM_POSTGRES_LIBICU7="libicu-50.2-4.el7_7.x86_64"
RPM_POSTGRES_LIBICU8="libicu-60.3-2.el8_1.x86_64"
RPM_POSTGRES_LIBICU9="libicu-67.1-9.el9.x86_64"

RPM_POSTGRES_LZ4="lz4-1.8.3-3.el8_4.x86_64"
RPM_POSTGRES_LIBZSTD7="libzstd-1.5.5-1.el7.x86_64"

rpmDeb_hook_createPostgresData() {
    export RPM_DEB_POSTGRES_PATH="${RPM_DEB_POSTGRES_PATH}"
    export RPM_DEB_POSTGRES_ROOT_USER="${RPM_DEB_POSTGRES_ROOT_USER}"
    local scriptName="createPostgresUsers.sh"
    local postgresqlScript="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${scriptName}"
    ...
    io_setOwnership "${postgresqlScript}" "${RPM_DEB_POSTGRES_ROOT_USER}" "${RPM_DEB_POSTGRES_ROOT_USER}" >&6 2>&1 \
        || errorExit "Failed to set ${postgresqlScript} ownership on ${postgresqlScript}"

    local tmpPostgres="/tmp/${scriptName}"

    cp -f "${postgresqlScript}" "${tmpPostgres}" || errorExit "Failed to copy ${postgresqlScript} to ${tmpPostgres}"

    # Execute this file from a directory which postgres user can access
    cd /tmp && su ${RPM_DEB_POSTGRES_ROOT_USER} -c "POSTGRES_PATH=${RPM_DEB_POSTGRES_PATH} PGPASSWORD=${RPM_DEB_POSTGRES_ROOT_PASSWORD} DB_PASSWORD=${RPM_DEB_POSTGRES_PASSWORD} bash ${tmpPostgres}" >&6 2>&1
...
(Choose 'Y' to retry, 'N' to abort or 'S' to skip this step)"
        ...
        if [[ ${users_choice} =~ n|N ]]; then
            ...
            warn "Files which can be used to seed ${POSTGRES_LABEL} are available at [${postgresqlScript}] \
Please seed ${POSTGRES_LABEL} before starting ${PRODUCT_NAME}" #Don't add a tab to this line
        else
            rpmDeb_hook_createPostgresData
        fi
    fi
...
# temporarily having this logic to set postgres location and postgres service name
setPostgresLocation() {
    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat)
            RPM_DEB_POSTGRES_LOCATION="/usr/pgsql-${POSTGRES_MAJOR_VER}"
            RPM_DEB_POSTGRES_SERVICE_NAME="postgresql-${POSTGRES_MAJOR_VER}"
        ;;
        SLES)
            RPM_DEB_POSTGRES_LOCATION="/usr/lib/postgresql${POSTGRES_SLES_MAJOR_VER}"
            RPM_DEB_POSTGRES_SERVICE_NAME="postgresql"
        ;;
        Debian|Ubuntu)
            RPM_DEB_POSTGRES_LOCATION="/usr/lib/postgresql/${POSTGRES_MAJOR_VER}"
            RPM_DEB_POSTGRES_SERVICE_NAME="postgresql"
        ;;
        *)
            errorExit "Distribution ${LINUX_DISTRIBUTION} is not supported"
        ;;
    esac
}

rpmDeb_postgresInit(){
    RPM_DEB_POSTGRES_HOME_KEY="installer.postgresql.home"
    RPM_DEB_POSTGRES_URL_KEY="${SYS_KEY_SHARED_DATABASE_URL}"
    RPM_DEB_POSTGRES_HOME_DEFAULT="/var/opt/postgres"
    setPostgresLocation
    RPM_DEB_POSTGRES_PATH="${RPM_DEB_POSTGRES_LOCATION}/bin"
    RPM_DEB_POSTGRES_ROOT_USER=${RPM_DEB_POSTGRES_ROOT_USER:-"postgres"}
    RPM_DEB_POSTGRES_ROOT_PASSWORD=${RPM_DEB_POSTGRES_ROOT_PASSWORD:-"postgres"}
    RPM_DEB_POSTGRES_SCRIPT_HOME="${RPM_DEB_INSTALLER_HOME}/third-party/postgresql"
    RPM_DEB_POSTGRES_PORT=5432
}

rpmDeb_isPostgresInstalled() {
    RPM_DEB_POSTGRES_ALREADY_INSTALLED="${FLAG_N}"
     local isSystemctlAvailable=$(hash systemctl 2>/dev/null; echo $?)
        if [[ "${isSystemctlAvailable}" == "0" ]]; then
            systemctl status "${RPM_DEB_POSTGRES_SERVICE_NAME}" 2>&1 | grep -E 'No such file or directory|could not be found|unrecognized service' -wq
        else
            service "${RPM_DEB_POSTGRES_SERVICE_NAME}" status 2>&1 | grep -E 'No such file or directory|could not be found|unrecognized service' -wq
        fi
    local POSTGRES_FOUND=$(echo $?)
    if [ ${POSTGRES_FOUND} != 0 ]; then
        RPM_DEB_POSTGRES_ALREADY_INSTALLED="${FLAG_Y}"
    fi
}
...
installPostgresRpm(){
    local postgresRpms=
    local postgresRpm=
    if [[ "${LINUX_DISTRIBUTION}" == "SLES" ]]; then
        eval postgres_pck="${POSTGRES_SLES}"
        eval postgres_server_pck="${POSTGRES_SERVER_SLES}"
        eval postgres_contrib_pck="${POSTGRES_CONTRIB_SLES}"
        postgresRpms="${postgres_pck} ${postgres_server_pck} ${postgres_contrib_pck}"
    else
        eval postgres_libicu_pack="\${RPM_POSTGRES_LIBICU${LINUX_DISTRIBUTION_VER}}"
        eval postgres_libs_pck="\${RPM_POSTGRES_LIBS${LINUX_DISTRIBUTION_VER}}"
        eval postgres_libzstd_pack="\${RPM_POSTGRES_LIBZSTD${LINUX_DISTRIBUTION_VER}}"

        eval postgres_pck="\${RPM_POSTGRES${LINUX_DISTRIBUTION_VER}}"
        eval postgres_server_pck="\${RPM_POSTGRES_SERVER${LINUX_DISTRIBUTION_VER}}"
        eval postgres_contrib_pck="\${RPM_POSTGRES_CONTRIB${LINUX_DISTRIBUTION_VER}}"

        local checkLibicuInstalled=$(rpm -qa | grep libicu >/dev/null; echo $?)

        if [[ "${checkLibicuInstalled}" == "0" ]]; then
            postgresRpms="${postgres_libs_pck} ${postgres_pck} ${postgres_server_pck} ${postgres_contrib_pck}"
        else
            postgresRpms="${postgres_libicu_pack} ${postgres_libs_pck} ${postgres_pck} ${postgres_server_pck} ${postgres_contrib_pck}"
        fi

        if [[ ${LINUX_DISTRIBUTION_VER} == "7" ]];then
            local checkLibzstdInstalled=$(installedLibzstd=`rpm -qa  libzstd` ; if [[ $installedLibzstd == "" ]]; then echo 1  ; else echo 0; fi)
            if [[ "${checkLibzstdInstalled}" != "0" ]]; then
                rpm -ivh --replacepkgs "${RPM_DEB_POSTGRES_SCRIPT_HOME}/${postgres_libzstd_pack}.rpm" >&6 2>&1 || errorExit "Installing ${RPM_POSTGRES_LIBZSTD}.rpm package failed"
            fi
        fi
    fi

    if [[ "$LINUX_DISTRIBUTION" != "SLES" && "$POSTGRES_MAJOR_VER" -ge "15" ]]; then
        local checkLz4Installed=$(installedLZ4=`rpm -qa  lz4` ; if [[ $installedLZ4 == "" ]]; then echo 1  ; else echo 0; fi)
        if [[ "${checkLz4Installed}" != "0" ]]; then
            rpm -ivh --replacepkgs "${RPM_DEB_POSTGRES_SCRIPT_HOME}/${RPM_POSTGRES_LZ4}.rpm" >&6 2>&1 || errorExit "Installing ${RPM_POSTGRES_LZ4}.rpm package failed"
        fi
    fi

    for postgresRpm in ${postgresRpms}; do
        local packFile="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${postgresRpm}.rpm"
        logger "Installing ${packFile} ..."
        if [[ "${LINUX_DISTRIBUTION}" == "SLES" ]]; then
          zypper install -y "${packFile}" >&6 2>&1 || errorExit "Installing ${packFile} package failed"
        else
            if [[ "$postgresRpm" == *"contrib"* ]]; then
            rpm -ivh --replacepkgs --nodeps "${packFile}" >&6 2>&1 || errorExit "Installing ${packFile} package failed"
            else
            rpm -ivh --replacepkgs "${packFile}" >&6 2>&1 || errorExit "Installing ${packFile} package failed"
            fi
        fi
    done
}

ensureLocaleIsPresent(){
    if locale -a | grep -xq en_US.utf8
    then
        logger "locale en_US.utf8 is present"
    else
        logger "locale en_US.utf8 not present, installing"
        apt-get install -y locales >&6 2>&1
        echo "" >> /etc/locale.gen
        echo "en_US.UTF-8 UTF-8" >> /etc/locale.gen
        locale-gen >&6 2>&1
        if locale -a | grep -xq en_US.utf8
        then
            logger "installed locale en_US.utf8"
        else
            errorExit "failed to install locale en_US.utf8"
        fi
    fi
}

installPostgresDeb(){
    local VERSION=$(awk -F'=' '/VERSION_ID/ {print $2}' /etc/os-release | tr -d '"' 2>/dev/null)
    logDebug "${LINUX_DISTRIBUTION} Version: ${VERSION}"

    if [ "${VERSION}" == "${UBUNTU18_VERSION}" ]; then
        packFile="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${UBUNTU18_POSTGRESQL}.deb"
        packName="${UBUNTU18_POSTGRESQL}.deb"
    elif [ "${VERSION}" == "${UBUNTU20_VERSION}" ]; then
        packFile="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${UBUNTU20_POSTGRESQL}.deb"
        packName="${UBUNTU20_POSTGRESQL}.deb"
    elif [ "${VERSION}" == "${UBUNTU22_VERSION}" ]; then
        packFile="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${UBUNTU22_POSTGRESQL}.deb"
        packName="${UBUNTU22_POSTGRESQL}.deb"
    elif [ "${VERSION}" == "${DEBIAN10_VERSION}" ]; then
        packFile="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${DEBIAN10_POSTGRESQL}.deb"
        packName="${DEBIAN10_POSTGRESQL}.deb"
    elif [ "${VERSION}" == "${DEBIAN11_VERSION}" ]; then
        packFile="${RPM_DEB_POSTGRES_SCRIPT_HOME}/${DEBIAN11_POSTGRESQL}.deb"
        packName="${DEBIAN11_POSTGRESQL}.deb"
    else
        errorExit "${LINUX_DISTRIBUTION} ${VERSION} is not supported"
    fi
     
    if [[ "${VERSION}" == "${DEBIAN10_VERSION}" || "${VERSION}" == "${DEBIAN11_VERSION}" ]]; then
        apt update -y >&6 2>&1
        apt-get install wget sudo -y >&6 2>&1
        apt-get install -y gnupg gnupg1 gnupg2 >&6 2>&1
        apt-get install -y lsb-release >&6 2>&1
        ensureLocaleIsPresent
    fi

    # Temporary fix for installing postgresql on ubuntu 20.x, until postgresql upgrade to 13.6
    if [[ "${VERSION}" == "${UBUNTU20_VERSION}" ]]; then
        apt-get install libllvm9 -y >&6 2>&1
    fi

    if [[ "${VERSION}" == "${UBUNTU22_VERSION}" ]]; then
        apt-get install libllvm14 -y >&6 2>&1
    fi

    logger "Installing (${packName}) ..."
    dpkg -i ${packFile} >&6 2>&1
    if [ $? -gt 0 ]; then
        setRepoConfig
        logger "Installing missing (${packName}) dependencies requires internet access, proceeding with installation..."
        apt-get update >&6 2>&1
        apt-get install -f -y >&6 2>&1 || errorExit "Installing missing ${packName} dependencies failed"
        apt-get update >&6 2>&1
...
setRepoConfig(){
    ...
    if [[ "${VERSION}" == "${UBUNTU18_VERSION}" ]]; then
        sh -c 'echo "deb https://apt-archive.postgresql.org/pub/repos/apt/ $(lsb_release -cs)-pgdg main" >> /etc/apt/sources.list.d/pgdg.list'
    else
        sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt/ $(lsb_release -cs)-pgdg main" >> /etc/apt/sources.list.d/pgdg.list'
    fi

    # Import the repository signing key:
    wget --no-check-certificate --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -  >&6 2>&1
}

unsetRepoConfig(){
    if [ -f /etc/apt/sources.list.origfile ]; then
    ...
    if [ -f /etc/apt/sources.list.d/pgdg.list.origfile ]; then
...
rpmDeb_checkPostgresNotInProduct() {
    local folder=${1}
    local productData=""

    [ ! -z "${folder}" ] || return

    getSystemValue "${KEY_DATA_DIR_LOCATION}" "${JF_PRODUCT_VAR_DEFAULT}" "false" "${INSTALLER_YAML}"
    productData="${YAML_VALUE}"

    VALID_POSTGRES_DATA_FOLDER="${FLAG_N}"
    #prevent creating $POSTGRES_LABEL data inside Mission Control data because of users permissions
    echo ${folder} | grep -E "^${productData}" 2>&1 >/dev/null
    if [ $? -eq 0 ]; then
        warn "folder ${POSTGRES_LABEL} cannot be inside ${PRODUCT_NAME} data folder"
    else
        VALID_POSTGRES_DATA_FOLDER="${FLAG_Y}"
    fi
}

installNewPostgres() {
    logger "Checking if $POSTGRES_LABEL already exists" >&6 2>&1

    getSystemValue "${RPM_DEB_POSTGRES_HOME_KEY}" "${RPM_DEB_POSTGRES_HOME_DEFAULT}" "false" "${INSTALLER_YAML}"
    local postgresHome="${YAML_VALUE}"

    rpmDeb_checkPostgresNotInProduct "${postgresHome}"
    local postgresData="${postgresHome}/data" 

    if [ ! -d "${postgresData}" ]; then
        logger "Creating ${POSTGRES_LABEL} data folder on ${postgresData}"
        createDir "${postgresData}" || errorExit "Could not create postgres ${postgresData} data directory"
    fi

    rpmDeb_isPostgresInstalled
    if [ ${RPM_DEB_POSTGRES_ALREADY_INSTALLED} != "${FLAG_Y}" ]; then
        logger "Installing/Verifying ${POSTGRES_LABEL} (this may take several minutes)..."
        
        case ${LINUX_DISTRIBUTION} in
            CentOS|RedHat|SLES)
                installPostgresRpm
                local postgresServiceFile=
                chown -R ${RPM_DEB_POSTGRES_ROOT_USER}:${RPM_DEB_POSTGRES_ROOT_USER} "${postgresData}" || errorExit "Could not set permission for ${postgresData} data directory"
                rm -rf ${RPM_DEB_POSTGRES_LOCATION}/share/man ${RPM_DEB_POSTGRES_LOCATION}/lib/*.a
                
                local isSystemctlAvailable=$(hash systemctl 2>/dev/null; echo $?)
                # Change the data directory
                export PGDATA="${postgresData}"
                export PGSETUP_INITDB_OPTIONS="-D ${postgresData}"
                if [[ "${isSystemctlAvailable}" == "0" ]]; then
                    if [[ "${LINUX_DISTRIBUTION}" == "SLES" ]]; then
                        local postgresConfigFile="/etc/sysconfig/postgresql"
                        sed -i "s~^POSTGRES_DATADIR=.*~POSTGRES_DATADIR=${postgresData}~" ${postgresConfigFile}
                    else
                        postgresServiceFile="/lib/systemd/system/${RPM_DEB_POSTGRES_SERVICE_NAME}.service"
                        sed -i "s~^Environment=PGDATA=.*~Environment=PGDATA=${postgresData}~" ${postgresServiceFile}
                    fi

                    systemctl daemon-reload
                    if [[ "${LINUX_DISTRIBUTION}" == "SLES" ]]; then
                        su ${RPM_DEB_POSTGRES_ROOT_USER} -c "${RPM_DEB_POSTGRES_PATH}/initdb --pgdata=${postgresData}" >&6 2>&1 || errorExit "Failed to initialize ${RPM_DEB_POSTGRES_PATH}/initdb using custom directory"
                    else
                        ${RPM_DEB_POSTGRES_PATH}/${RPM_DEB_POSTGRES_SERVICE_NAME}-setup initdb
                    fi
                else
                    sed -i "s~^PGDATA=.*~PGDATA=${postgresData}~" /etc/init.d/${RPM_DEB_POSTGRES_SERVICE_NAME}
                    service ${RPM_DEB_POSTGRES_SERVICE_NAME} initdb
                fi
                rpmDeb_autoRestartPostgresqlOnSystemReboot
                replaceText "ident" "trust" "${postgresData}/pg_hba.conf"
                replaceText "scram-sha-256" "trust" "${postgresData}/pg_hba.conf"
                replaceText "peer" "trust" "${postgresData}/pg_hba.conf"
            ;;
            Debian|Ubuntu)
                local originalPostgresConf="/etc/postgresql/${POSTGRES_MAJOR_VER}/main/postgresql.conf"
                installPostgresDeb
                chown -R ${RPM_DEB_POSTGRES_ROOT_USER}:${RPM_DEB_POSTGRES_ROOT_USER} "${postgresData}" || errorExit "Could not set permission for ${postgresData} data directory"
                rm -rf ${RPM_DEB_POSTGRES_LOCATION}/lib/pgxs

                local isSystemctlAvailable=$(hash systemctl 2>/dev/null; echo $?)
                if [[ "${isSystemctlAvailable}" == "0" ]]; then
                    systemctl stop ${RPM_DEB_POSTGRES_SERVICE_NAME}.service
                    # Change the data directory
                    sed -i "s~^data_directory =.*~data_directory = '${postgresData}'~" "${originalPostgresConf}"
                    sed -i "s~^hba_file =.*~hba_file = '${postgresData}/pg_hba.conf'~" "${originalPostgresConf}"
                    sed -i "s~^ident_file =.*~ident_file = '${postgresData}/pg_ident.conf'~" "${originalPostgresConf}"
                fi
                addLine "listen_addresses='*'" "${originalPostgresConf}"
                su ${RPM_DEB_POSTGRES_ROOT_USER} -c "${RPM_DEB_POSTGRES_PATH}/initdb --pgdata=${postgresData}" >&6 2>&1 || errorExit "Failed to initialize ${RPM_DEB_POSTGRES_PATH}/initdb using custom directory"
                rpmDeb_autoRestartPostgresqlOnSystemReboot
            ;;
            *)
        ...
        addLine "host    all             all             0.0.0.0/0               md5" "${postgresData}/pg_hba.conf"
            
        # Add the following line to ${LINUX_POSTGRES_DATA}/postgresql.conf
        addLine "listen_addresses='*'" "${postgresData}/postgresql.conf"
        addLine "port=${RPM_DEB_POSTGRES_PORT}" "${postgresData}/postgresql.conf"
    else
        logger "Package ${POSTGRES_LABEL} is already installed. Skipping installation"
    fi
    
    # Stop postgress if it is already running under /var/opt/postgres/data
    [ -f "${postgresData}/postmaster.pid" ] && cat "${postgresData}/postmaster.pid" | head -1 | xargs kill -9
    rpmDeb_startService "${RPM_DEB_POSTGRES_SERVICE_NAME}"

    sleep 5

    rpmDeb_hook_createPostgresData
    if [ -f ${postgresData}/pg_hba.conf ]; then
        replaceText " trust" " md5" "${postgresData}/pg_hba.conf"
        service "${RPM_DEB_POSTGRES_SERVICE_NAME}" restart
    fi
}

rpmDeb_isPostgresInstallEnabled(){
    : ${INSTALLER_POSTGRESQL_INSTALLED:="NOT_SET"}
    
    if [ "${INSTALLER_POSTGRESQL_INSTALLED}" == "NOT_SET" ]; then
        getSystemValue "${KEY_POSTGRES_INSTALL}" "NOT_SET" "false" "${INSTALLER_YAML}"
        INSTALLER_POSTGRESQL_INSTALLED=${YAML_VALUE}
    fi

    local postgresEnabled=""

    [ "${INSTALLER_POSTGRESQL_INSTALLED}" == "NOT_SET" ] && postgresEnabled="${FLAG_N}" || postgresEnabled="${INSTALLER_POSTGRESQL_INSTALLED}"

    if [[ ${postgresEnabled} =~ y|Y ]]; then
        return 0;
    else
        return 1;
    fi
...
installPostgres() {
    bannerStart "${POSTGRES_LABEL} Installation"
    rpmDeb_postgresInit
    local sytemYamlMsg=
    if [[ "${TRIAL_FLOW}" == "${FLAG_Y}" ]]; then
        sytemYamlMsg="Connection details can be modified in $JF_PRODUCT_HOME_RT/var/etc/system.yaml and $JF_PRODUCT_HOME_XRAY/var/etc/system.yaml after installation"
    else
        sytemYamlMsg="Connection details can be modified in $JF_PRODUCT_HOME/var/etc/system.yaml after installation"
    fi
    
    if [ "${RPM_DEB_IS_UPGRADE}" == true ]; then
        logger "Skipping ${POSTGRES_LABEL} installation as this is an upgrade"
        logger "${sytemYamlMsg}"
        checkPostgreSQLContribLibraries

    elif rpmDeb_isPostgresInstallEnabled; then
        installNewPostgres
    else
        logger "Skipping ${POSTGRES_LABEL} installation" >&6 2>&1
    fi
}

# This function is used to check the needed postgresql-contrib packages (to support pg_trgm) from xray version 3.70.x onwards
function checkPostgreSQLContribLibraries {
    if [[ "$LINUX_DISTRIBUTION" == *"CentOS"* ]] || [[ "$LINUX_DISTRIBUTION" == *"RedHat"* ]] || [[ "$LINUX_DISTRIBUTION" == *"SLES"* ]]; then
        rpmDeb_isPostgresInstalled
        if [ ${RPM_DEB_POSTGRES_ALREADY_INSTALLED} == "${FLAG_Y}" ]; then
            output=$(rpm -qa | grep postgresql)
            contribLibs="false"
            while read -r line; do
                if echo "$line" | grep 'contrib'; then
                contribLibs="true"
                fi
            done <<< "$output"
            if [[ $contribLibs == "false" ]]; then
                setYellow "The postgresql-contrib packages are required from xray versions 3.70.x."
                setYellow "For further details, please visit the documentation link: https://jfrog.com/help/r/jfrog-installation-setup-documentation/support-for-pg_trgm-module"
            fi
        fi
    fi
}

# Autostart postgresql service on system reboot
rpmDeb_autoRestartPostgresqlOnSystemReboot () {

    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat|SLES)
            if [ -f "/etc/init.d/${RPM_DEB_POSTGRES_SERVICE_NAME}" ]; then
                chkconfig --add ${RPM_DEB_POSTGRES_SERVICE_NAME}
                chkconfig ${RPM_DEB_POSTGRES_SERVICE_NAME} on
            elif [ -f "/usr/lib/systemd/system/${RPM_DEB_POSTGRES_SERVICE_NAME}.service" ] || [ -f "/etc/systemd/system/${RPM_DEB_POSTGRES_SERVICE_NAME}.service" ]; then
                systemctl enable ${RPM_DEB_POSTGRES_SERVICE_NAME}
            fi
        ;;
        Debian|Ubuntu)
            if [ -f "/etc/init.d/${RPM_DEB_POSTGRES_SERVICE_NAME}" ]; then
...
CENTOS_ERLANG7="esl-erlang_${RPM_DEB_ERLANG_VERSION}-1.el7.x86_64"
CENTOS_ERLANG8="erlang-${RPM_DEB_ERLANG_VERSION}-1.el8.x86_64"
CENTOS_ERLANG9="erlang-${RPM_DEB_ERLANG_VERSION}-1.el9.x86_64"
UBUNTU18_ERLANG="esl-erlang_${RPM_DEB_ERLANG_VERSION}-1_ubuntu_bionic_amd64"
UBUNTU20_ERLANG="esl-erlang_${RPM_DEB_ERLANG_VERSION}-1_ubuntu_focal_amd64"
UBUNTU22_ERLANG="esl-erlang_${RPM_DEB_ERLANG_VERSION}-1_ubuntu_jammy_amd64"
DEBIAN10_ERLANG="esl-erlang_${RPM_DEB_ERLANG_VERSION}-1_debian_buster_amd64"
DEBIAN11_ERLANG="esl-erlang_${RPM_DEB_ERLANG_VERSION}-1_debian_bullseye_amd64"

CENTOS_SOCAT9="socat-1.7.4.1-5.el9.x86_64"
CENTOS_SOCAT8="socat-1.7.3.3-2.el8.x86_64"
CENTOS_SOCAT7="socat-1.7.3.2-2.el7.x86_64"
DEBIAN_SOCAT="socat_1.7.4.1-3_amd64"

CENTOS7_DB_53_UTILS="libdb-utils-5.3.21-25.el7.x86_64"
CENTOS9_DB_53_UTILS="libdb-utils-5.3.28-53.el9.x86_64"

CENTOS7_UNIX_ODBC="unixODBC-2.3.1-14.el7.x86_64"
CENTOS7_LIBLTDL="libltdl7-2.4.6-3.4.1.x86_64"
DEBIAN10_DB_53_UTIL="db-util_5.3.1+nmu1_all"

DEBIAN_DB_53_UTIL="db-util_5.3.0_all"
UBUNTU18_DB_53_UTIL="db5.3-util_5.3.28-13.1ubuntu1.1_amd64"
UBUNTU20_DB_53_UTIL="db5.3-util_5.3.28+dfsg1-0.6ubuntu2_amd64"
UBUNTU22_DB_53_UTIL="db5.3-util_5.3.28+dfsg1-0.8ubuntu3_amd64"
UBUNTU_DB_UTIL="db-util_1_3a5.3.21exp1ubuntu1_all"

installRabbitmqDep() {
    local packName="${1}"
    local packFile="${2}"

    rpmDeb_isPackInstalled "${packName}" "${packFile}"
    if [ $? -eq 0 ]; then
        logDebug "Skipping ${packName} Installation"
        return 0
    fi
...
installSocatRHEL(){
    local packName="socat"
    eval socat_pck="\${CENTOS_SOCAT${LINUX_DISTRIBUTION_VER}}"
    local packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${socat_pck}.rpm"
    rpmDeb_isPackInstalled "${packName}" "${packFile}"
    if [ $? -eq 0 ]; then
        logDebug "Skipping ${packName} Installation"
        return
    ...
    logger "Installing ${packFile} ..."
    rpm -ivh --replacepkgs ${packFile} >&6 2>&1 || errorExit "Installing ${packName} package failed"
}

installErlangRHEL(){
    local packName="erlang"
    eval erlang_pck="\${CENTOS_ERLANG${LINUX_DISTRIBUTION_VER}}"
    local packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${erlang_pck}.rpm"

    oldRabbit=$(rpm -qa | grep rabbitmq-server-3.6.1-1)
    if [ ! -z "${oldRabbit}" ]; then
        oldErlang=$(rpm -qa | grep erlang)
        rpm -e "${oldRabbit}" || errorExit "Uninstalling ${oldRabbit} package failed"
        if [ ! -z "${oldErlang}" ]; then
            rpm -e "${oldErlang}" || errorExit "Uninstalling ${oldErlang} package failed"
        fi
    fi
    ...
    if [ ${LINUX_DISTRIBUTION_VER} == 7 ]; then
        ...
        # Remove old erlang package if it exists as we will upgrade to esl-erlang package
        erlangExists=$(rpm -qa erlang)
        if [ ! -z "${erlangExists}" ]; then
           removeOldErlang
        fi
    fi

    logger "Installing ${packFile} ..."
    rpm -Uvh --replacepkgs "${packFile}" >&6 2>&1 || errorExit "Installing ${packName} package failed"
}

removeOldErlang() {
   # Remove old version of erlang as we will be using esl-erlang

   beam_smp_pid=$(ps -ef | grep -v grep | grep -w beam.smp | awk '{print $2}')
   if [[ ${#beam_smp_pid} -ge 1 ]]; then
      kill -9 ${beam_smp_pid}
   fi

   epmd_pid=$(ps -ef | grep -v grep | grep -w epmd | awk '{print $2}')
   if [[ ${#epmd_pid} -ge 1 ]]; then
      kill -9 ${epmd_pid}
   fi
   logger "Uninstalling existing erlang ..."
   rpm --nodeps -e erlang || errorExit "Uninstalling erlang package failed"
}

installDbUtilRPM(){
    local packFile=
    local packName="libdb-utils"
    
    packFile="${RPM_DEB_THIRDPARTY_HOME}/misc/${CENTOS7_DB_53_UTILS}.rpm"
    if [ ${LINUX_DISTRIBUTION_VER} == 9 ]; then
        packFile="${RPM_DEB_THIRDPARTY_HOME}/misc/${CENTOS9_DB_53_UTILS}.rpm"
    fi
    # Install db-util if db_dump is not available
    if ! hash db_dump >/dev/null 2>&1; then
        rpm -ivh --replacepkgs "${packFile}" >&6 || errorExit "Installing ${packName} package failed"
    else
        logDebug "Skipping ${packName} Installation"
    fi
}
...
installLibtdl(){
    local packName="libltdl"
    local packFile="${RPM_DEB_THIRDPARTY_HOME}/rabbitmq/${CENTOS7_LIBLTDL}.rpm"
    rpmDeb_isPackInstalled "${packName}" "${packFile}"
    if [ $? -eq 0 ]; then
        logDebug "Skipping ${packName} Installation"
        return
    ...
    logger "Installing ${packFile} ..."
    rpm -ivh --replacepkgs ${packFile} >&6 2>&1 || echo "Installing ${packName} package failed. Can be ignored if unixODBC installation passes successfully."
}

installUnixODBC(){
    local packName="unixODBC"
    local packFile="${RPM_DEB_THIRDPARTY_HOME}/rabbitmq/${CENTOS7_UNIX_ODBC}.rpm"
...
installDbUtil() {
    logDebug "Verifying db-util package existence for Berkeley DB ..."
    local dbutilFiles="${RPM_DEB_THIRDPARTY_HOME}/misc"
    local packName=""
    ...
    case ${LINUX_DISTRIBUTION} in
        ...
        Debian)
            ...
            if [ "${VERSION}" == "${DEBIAN10_VERSION}" ] || [ "${VERSION}" == "${DEBIAN11_VERSION}" ]; then
                packFile="${dbutilFiles}/${DEBIAN10_DB_53_UTIL}.deb"

            else
                errorExit "${LINUX_DISTRIBUTION} ${VERSION} is not supported"
            fi

            installRabbitmqDep "${packName}" "${packFile}"

            packName="db-util"
            packFile="${dbutilFiles}/${DEBIAN_DB_53_UTIL}.deb"
            installRabbitmqDep "${packName}" "${packFile}"
        ;;
        Ubuntu)
            packName="db5.3-util"
            ...
            if [ "${VERSION}" == "${UBUNTU18_VERSION}" ]; then
                packFile="${dbutilFiles}/${UBUNTU18_DB_53_UTIL}.deb"
            elif [ "${VERSION}" == "${UBUNTU20_VERSION}" ]; then
                packFile="${dbutilFiles}/${UBUNTU20_DB_53_UTIL}.deb"
            elif [ "${VERSION}" == "${UBUNTU22_VERSION}" ]; then
                packFile="${dbutilFiles}/${UBUNTU22_DB_53_UTIL}.deb"
            else
                errorExit "${LINUX_DISTRIBUTION} ${VERSION} is not supported"
            fi
            ...
            packName="db-util"
            packFile="${dbutilFiles}/${UBUNTU_DB_UTIL}.deb"
            installRabbitmqDep "${packName}" "${packFile}"
        ;;
        *)
            errorExit "Distribution $LINUX_DISTRIBUTION is not supported"
...
installErlangDeb() {
    ...
    if [ "${VERSION}" == "${UBUNTU18_VERSION}" ]; then
        packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${UBUNTU18_ERLANG}.deb"
    elif [ "${VERSION}" == "${UBUNTU20_VERSION}" ]; then
        packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${UBUNTU20_ERLANG}.deb"
    elif [ "${VERSION}" == "${UBUNTU22_VERSION}" ]; then
        packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${UBUNTU22_ERLANG}.deb"
    elif [ "${VERSION}" == "${DEBIAN10_VERSION}" ]; then
        packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${DEBIAN10_ERLANG}.deb"
    elif [ "${VERSION}" == "${DEBIAN11_VERSION}" ]; then
        packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${DEBIAN11_ERLANG}.deb"
    else
        errorExit "${LINUX_DISTRIBUTION} ${VERSION} is not supported"
    fi
    installRabbitmqDep "${packName}" "${packFile}"
...
installRabbitmqDependencies() {
    ...
    ### Skipping the installation of Rabbitmq dependencies needs further changes in the Omnibus build process.
    if [[ "${INSTALLATION_METHOD}" != "xray" ]]; then
        bannerStart "RabbitMQ dependencies Installation"
        logger "Installing/Verifying RabbitMQ dependencies (this may take several minutes)..."
    fi

    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat)
            local socatExists=$(rpm -qa socat)
            if [ -z "${socatExists}" ]; then
                installSocatRHEL
            fi
            installErlangRHEL
            installDbUtil
        ...
        Debian|Ubuntu)
            ...
            packName="socat"
            packFile="${RPM_DEB_RABBITMQ_DEPENDENICES_DIR}/${DEBIAN_SOCAT}.deb"
            if [ "${VERSION}" != "${UBUNTU22_VERSION}" ]; then
                installRabbitmqDep "${packName}" "${packFile}"
            fi
            installErlangDeb
...
WRAPPER_SCRIPT_TYPE="$WRAPPER_SCRIPT_TYPE_RPMDEB"

rpmDeb_isUseDefaultsEnabled() {
...
rpmDeb_checkLinuxDistribution() {
    validateSudo
    io_getLinuxDistribution

    # If not set - not supported
    if [ -z "${LINUX_DISTRIBUTION}" ]; then errorExit "Linux distribution type not supported"; fi

    if [[ "${LINUX_DISTRIBUTION}" == "Debian" ]] && [[ "${PRODUCT_NAME}" == "Distribution" ]] && [[ $(cat /etc/*release 2>/dev/null | grep "VERSION_ID=\"9\"" 2>/dev/null) == *"VERSION_ID"* ]];
    then
        errorExit "${LINUX_DISTRIBUTION} 9 is not supported."
    fi

    if [[ "${LINUX_DISTRIBUTION}" != "Debian" && "${LINUX_DISTRIBUTION}" != "Ubuntu" && "${LINUX_DISTRIBUTION}" != "CentOS" && "${LINUX_DISTRIBUTION}" != "RedHat" && "${LINUX_DISTRIBUTION}" != "SLES" ]];
    then
        errorExit "${LINUX_DISTRIBUTION} is not supported. Debian, Ubuntu, CentOS, RedHat and its variants are supported"
    fi
...
rpmDeb_isInstalled(){
    ...
    case ${LINUX_DISTRIBUTION} in
        ...
        ;;
        *)
            errorExit "isInstalled: Distribution ${LINUX_DISTRIBUTION} is not supported"
            return 1
        ;;
    esac
...
rpmDeb_isPackInstalled() {
    local packName=$1
    local packFile=$2
    local strictCheck=$3
    if [[ ! -f "$packFile" ]]
    then
        errorExit "File ${packFile} not found, please verify if all the extracted files are available. If not extract and try again"
    fi
    local currentName=''
    local newName=''
    local currentVer=''
    ...
    local currentRel=''
    local newRel=''
    local productPackName='${RPM_DEB_PRODUCT_PACK_NAME}'

    if [[ -z "${packName}" ]] || [[ "${packName}" == "" ]] || [[ -z "${packFile}" ]] || [[ "${packFile}" == "" ]]
    then
        errorExit "Method rpmDeb_isPackInstalled: missing packName\packFile value"
    fi

    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat|SLES)
            currentName="$(rpm -qa --nosignature --queryformat '%{NAME}\n' ${packName} 2>/dev/null)"
            currentVer="$(rpm -qa --nosignature --queryformat '%{VERSION}\n' ${packName} 2>/dev/null)"
            currentRel="$(rpm -qa --nosignature --queryformat '%{RELEASE}\n' ${packName} 2>/dev/null)"

            newName="$(rpm -qp --nosignature --queryformat '%{NAME}\n' ${packFile} 2>/dev/null)"
            newVer="$(rpm -qp --nosignature --queryformat '%{VERSION}\n' ${packFile} 2>/dev/null)"
            newRel="$(rpm -qp --nosignature --queryformat '%{RELEASE}\n' ${packFile} 2>/dev/null)"
        ;;
        Debian|Ubuntu)
            if [[ "$strictCheck" == *"yes"* ]]; then
                ...
                status=$(dpkg -l "${packName}" 2>/dev/null | grep "${packName}" | awk '{print $1}')
        ...
        ;;
        *)
            errorExit "Method rpmDeb_isPackInstalled: Distribution ${LINUX_DISTRIBUTION} is not supported"
            return 1
        ;;
    esac

    # replace ~ with a dot - version comparison fails for ~
    ...
    if [ "${packName}" == "${productPackName}" ] && [[ "${currentVer}" == *[b]* ]]; then
    ...
    #support for dev upgrades
    if [ "${packName}" == "${productPackName}" ] && [[ "${newVer}" == *[b]* ]]; then
        newVer=$(echo "${newVer,,}" | sed 's/b//g' )
    fi

    #support for any version which contains string 
    # example : 2.2.0-m001
    #           3.0.0-rc001
    if [ "${packName}" == "${productPackName}" ] && [[ "${currentVer}" =~ ~*-*[a-zA-Z]+ ]]; then
        currentVer=$(echo "${currentVer,,}" | sed 's/-*~*[a-z][a-zA-Z]*/./g' )
        #handle if multiple dots are generated
        currentVer=$(echo "${currentVer}" | sed 's/\.\.\.*/./g' )
    fi
    ...
    # example : 2.2.0-m001
    #           3.0.0-rc001
    if [ "${packName}" == "${productPackName}" ] && [[ "${newVer}" =~ ~*-*[a-zA-Z]+ ]]; then
        newVer=$(echo "${newVer,,}" | sed 's/-*~*[a-z][a-zA-Z]*/./g' )
        #handle if multiple dots are generated
        newVer=$(echo "${newVer}" | sed 's/\.\.\.*/./g' )
    fi

    if [ "${currentName}" == "${newName}" ]; then
        rpmDeb_verComp "${currentVer}" "${newVer}" 2>/dev/null
        verCompResult=$?
        if [ ${verCompResult} -eq 0 ] || [ ${verCompResult} -eq 1 ]; then
            # compare release numbers
            rpmDeb_verComp "${currentRel}" "${newRel}" 2>/dev/null
            relCompResult=$?
            
            if [ ${relCompResult} -eq 0 ] || [ ${relCompResult} -eq 1 ]; then
                logger "Packname ${packName} [$currentVer] version is already installed" >&6 2>&1
            elif [ ${relCompResult} -eq 2 ]; then
                logger "Packname ${packName} [$currentVer] ($currentRel) version is installed, going to upgrade to [$newVer] ($newRel) version" >&6 2>&1
                return 2
            fi

        elif [ ${verCompResult} -eq 2 ]; then
            [ -z "$currentVer" ] || \
                logger "Packname ${packName} [$currentVer] version is installed, going to upgrade to [$newVer] version"
            return 2
        fi
    else
...
# usage : rpmDeb_getPackName "libvpx1_1.3.0-3_amd64.deb"
rpmDeb_getPackName() {
    local file="$1"
    if [ ! -f $file ]
    then
        errorExit "${file} not found, please verify if all the extracted files are available. If not extract and try again"
    fi
    if [ ! -z $file ]; then
        if [[ $file == *.deb ]]; then
            echo $(dpkg --info ${file} | grep Package | awk -F ': ' '{print $2}')
        elif [[ $file == *.rpm ]]; then
            echo $(rpm -qp --nosignature --queryformat '%{NAME}\n' ${file} 2>/dev/null)
        fi
    else
        echo ""
    fi
...
rpmDeb_invokeSystemUnInstaller() {
    packName=$1
    case ${LINUX_DISTRIBUTION} in
    CentOS|RedHat|SLES)
        yum erase -y $packName 2>/dev/null
    ;;
    Debian|Ubuntu)
        DEBIAN_FRONTEND=noninteractive apt purge -y $packName 2>/dev/null
    ;;
    *)
        errorExit "Distribution ${LINUX_DISTRIBUTION} is not supported"
    ;;
...
# Generic method to install deb/rpm package using package name
# parameters :
#   packFile        file to be installed
#   strictCheck      if yes is passed, a thorough check of install will be done
#   mode             install/upgrade , by default install will be executed
#   upgradeMode      set this to skip if you do not want to upgrade if installed
#   exitOnFailure    set to yes by default, set it no or anything else if you to continue on failure
#   extraOptions     pass any extra flags to be passed during install
# usage : rpmDeb_installPackage "somepackage.deb" "lenient_check" "install_mode" "force_upgrade" "yes" "--force-confnew"
rpmDeb_installPackage() {
    local packFile=$1
    local strictCheck=$2
    local mode=$3
    local upgradeMode=$4
    ...
    logDebug "Installing pack file: $packFile"
    local packName=$(rpmDeb_getPackName ${packFile})
    logDebug "Installing packName: $packName"

    [ ! -z "${packName}" ] || errorExit "Installing ${packFile} failed, could not determine packname using package"

    rpmDeb_isPackInstalled "${packName}" "${packFile}" "${strictCheck}"

    if [ $? -eq 0 ]; then
        logger "Skipping ${packName} installation" >&6 2>&1
    elif [[ $? -eq 2 ]] && [[ ! -z "$upgradeMode" ]] && [[ "$upgradeMode" == *"skip"* ]]; then 
        logger "Skipping ${packName} installation" >&6 2>&1
    elif [[ $packFile == *.deb ]]; then
        logDebug "Installing as deb"
        dpkg -i ${extraOptions} ${packFile} >&6
        if [[ $? -ne 0 ]]; then
            [[ $exitOnFailure == "yes" ]] && errorExit "Installing ${packName} package failed" || echo "Installing ${packName} package failed, port might already be allocated" >&6
        fi
    elif [[ $packFile == *.rpm ]]; then
        logDebug "Installing as rpm"
        if [[ "${INSTALLATION_METHOD}" == "rabbitmq" ]]; then
            rpm -${rpmOptions} ${extraOptions} --nosignature --replacepkgs --noscripts ${packFile} >&6
        else
            rpm -${rpmOptions} ${extraOptions} --nosignature --replacepkgs ${packFile} >&6
        fi
        if [[ $? -ne 0 ]]; then
            [[ $exitOnFailure == "yes" ]] && errorExit "Installing ${packName} package failed" || echo "Installing ${packName} package failed, port might already be allocated" >&6
        fi
...
# usage : rpmDeb_removePackage "package name" 
rpmDeb_removePackage() {
    local packageName=$1
...
rpmDeb_startService() {
    ...
    logDebug "Command to check status: $status_command"
    ...
    if [ ${attempstat} -gt 0 ]; then
        logError "failed to start [$packName]. This may be temporary. Try manually starting the service using [$start_command] and "
        errorExit "verifying its status using [$status_command]. Once it starts successfully, rerun the installer"
    fi
}

rpmDeb_checkPrerequisitesSoftware() {
    ...
    if [ ! -z "${unavailableTools}" ]; then
        logError "Some dependencies the installer needs are missing"
        logError "Please install them using [${command_to_run}] and then try again"
        errorExit "NOTE: You may need to tweak the list of sources in some cases"
    fi
}
...
rpmDeb_setYQPath() {
    logDebug "Method rpmDeb_setYQPath"
    if [ ! -z "${RPM_DEB_INSTALLER_HOME}" ] && [ -d "${RPM_DEB_INSTALLER_HOME}" ]; then
        _findYQ "${RPM_DEB_INSTALLER_HOME}"
    fi
...
rpmDeb_createLogFile() {
    redirectLogsToFile #creates variable DESTINATION_LOG_FILE
    INSTALLATION_LOG_FILE="${RPM_DEB_INSTALLER_HOME}/install.log"
    touch "${SYSTEM_DIAGNOSTICS_LOG_FILE}"
    configureLogOutput
}

rpmDeb_getPackVersion(){
    local packName=$1
...
rpmDeb_setVarSymlink(){
    local symlink="${JF_PRODUCT_HOME}/var"

    # Readlink will give the same path as response if it does not exist
    local varDir=$(readlink -f "${symlink}" 2>/dev/null)

    if [[ "${varDir}" == *"${symlink}"* ]]; then
        logger "Symlink for data directory not found" >&6 2>&1
    else
        RPM_DEB_DATA_DIR="${varDir}"
        return 0
    fi
}

rpmDeb_setProductDataEnv(){
    local dataDir="${RPM_DEB_DATA_DIR}"

    # If we are unable to discover current data directory by ourself, get it from installer yaml 
    if [ -z "${dataDir}" ]; then
        getSystemValue "${KEY_DATA_DIR_LOCATION}" "NOT_SET" "false" "${INSTALLER_YAML}"
        dataDir="${YAML_VALUE}"
    fi

    if [ -z "${dataDir}" ] || [ "${dataDir}" == "NOT_SET" ]; then
        return 0
    else
        addOrReplaceProperty "export JF_PRODUCT_VAR" "${dataDir}" "${RPM_DEB_DEFAULT_ENV}" "="
    fi
}

rpmDeb_installProduct() {
    local packFile=""
    bannerStart "${PRODUCT_NAME} Installation"
    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat|SLES)
            packFile=$(ls ${RPM_DEB_PRODUCT_FILE}*.rpm)
        ;;
        Debian|Ubuntu)
            packFile=$(ls ${RPM_DEB_PRODUCT_FILE}*.deb)
        ;;
        *)
            errorExit "Distribution ${LINUX_DISTRIBUTION} is not supported"
        ;;
    ...
    logger "Installing/Verifying ${PRODUCT_NAME} (${packFile}) (this may take several minutes)..."

    # pass 'yes' do a thorough check before install 
    rpmDeb_installPackage "${packFile}" "yes" "upgrade"
...
rpmDeb_isUpgrade() {
    ...
    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat|SLES)
            local currentVersion=$(rpm -qa --nosignature --queryformat '%{VERSION}\n' ${packName} 2>/dev/null)
            if [[ ! -z "${currentVersion}" && "${currentVersion}" != "" ]]; then
                RPM_DEB_IS_UPGRADE=true
                IS_UPGRADE="${FLAG_Y}"
                RPM_DEB_PRODUCT_INSTALLED_VER="${currentVersion}"
            fi
        ;;
        Debian|Ubuntu)
            # Check if this is an upgrade
            ...
            if [[ "$currentState" == "i" ]]; then
                RPM_DEB_IS_UPGRADE=true
                IS_UPGRADE="${FLAG_Y}"
                RPM_DEB_PRODUCT_INSTALLED_VER=$(dpkg-query -W -f='${Version}' ${packName} 2>/dev/null)
            fi
        ;;
        *)
            errorExit "Distribution ${LINUX_DISTRIBUTION} is not supported"
    ...
    if [ "${RPM_DEB_IS_UPGRADE}" == "true" ] && [ ! -z "${RPM_DEB_MIN_VERSION}" ] && [ ! -z "${RPM_DEB_MIN_VERSION_ERR_MSG}" ]; then
        rpmDebIsUpgradeAllowed "${packName}" "${RPM_DEB_MIN_VERSION}" "${RPM_DEB_MIN_VERSION_ERR_MSG}"
    fi 
}
...
-v | --verbose                                    : [optional] run installer in verbose mode
-h | --help                                       : [optional] display usage
-t | --type                                       : [optional] The installation method now supports either xray or rabbitmq. For example, use -t rabbitmq or -t xray

END_USAGE
    sleep .5
}

rpmDeb_processOptions() {
    VERBOSE_MODE=false 
...
rpmDeb_startLog(){
    logger "This script will install ${PRODUCT_NAME} and its dependencies."
    logger "After installation, logs can be found at ${INSTALLATION_LOG_FILE}"
}

rpmDeb_startLogRabbit(){
    logger "This script will install ${PRODUCT_NAME} rabbitmq and its dependencies."
    logger "After installation, logs can be found at ${INSTALLATION_LOG_FILE}"
}

rpmDeb_checkUserEndInstalls(){
    local userEndInstalls
    case ${LINUX_DISTRIBUTION} in
        CentOS|RedHat|SLES)
            userEndInstalls="$RPM_DEB_THIRDPARTY_DEP_CENTOS"
        ;;
        Debian)
            userEndInstalls="$RPM_DEB_THIRDPARTY_DEP_DEBIAN"
        ;;
        Ubuntu)
            userEndInstalls="$RPM_DEB_THIRDPARTY_DEP_UBUNTU"
        ;;
    esac
	rpmDeb_checkPrerequisitesSoftware "$userEndInstalls"    
}

rpmDeb_thirdPartyInstall (){
    for installCommand in ${RPM_DEB_THIRDPARTY_LIBRARIES}
    do
        logger "Invoking ${installCommand}" >&6 2>&1
        eval "${installCommand}"
    done
}
...
rpmDeb_endInfo(){
    echo ""
    logger "Installation complete. Installation log can be found at [${INSTALLATION_LOG_FILE}]"

    displayRpmDebStartInfo "${RPM_DEB_PRODUCT_SERVICE_NAME}" "${PRODUCT_NAME}"

    getJFrogURLIfAvailable >/dev/null || true

    echo ""
    if [ ! -z "${JF_SHARED_JFROGURL}" ]; then
        logger "Once the application has started, it can be accessed at [${JF_SHARED_JFROGURL}]"
    fi
    echo ""
}

rpmDeb_endInfoRabbitmq(){
    echo ""
    logger "Installation complete. Installation log can be found at [${INSTALLATION_LOG_FILE}]"

    displayRpmDebStartInfo "xray-rabbitmq" "XrayRabbitmq"
...
rpmDeb_copyYaml(){
    local mode="${1:-"pre"}"
    local installerYaml="${FILE_NAME_INSTALLER_YAML}"
    local systemYaml="system.yaml"

    local dataDir="${JF_PRODUCT_HOME}/var/etc/"

    if [ "${mode}" == "pre" ]; then
        [ -f "${dataDir}/${installerYaml}" ] && copyFile "${dataDir}/${installerYaml}" "${INSTALLER_YAML}" || true
    elif [ "${mode}" == "post" ]; then
        [ -f "${INSTALLER_YAML}" ] && copyFile "${INSTALLER_YAML}" "${dataDir}/${installerYaml}" || true
    else
        warn "Unsupported mode passed to rpmDeb_copyYaml"
    fi
}

rpmDeb_intializeLogFileName(){
    local parentPath=${1:-"${SCRIPT_HOME}"}
    local scriptHome="$(basename $0 | awk -F '.' '{print $1}')"
    local currentDate=$(date +"%Y%m%d%H%M")
    local logFileName="${scriptHome}.${currentDate}.log"
    INSTALLATION_LOG_FILE="${parentPath}/${logFileName}"
}

rpmDeb_init(){
    RPM_DEB_IS_UPGRADE="false"
    RPM_DEB_KEY_SYSTEM_TO_INSTALLER="systemToInstaller"
    RPM_DEB_KEY_INSTALLER_TO_SYSTEM="installerToSystem"
    JF_SYSTEM_YAML=${JF_PRODUCT_HOME}/var/etc/"${FILE_NAME_SYSTEM_YAML}"
    INSTALLER_STATE_YAML_FILE=${JF_PRODUCT_HOME}/var/etc/"${FILE_NAME_INSTALLER_STATE_YAML}"
    SYSTEM_DIAGNOSTICS_LOG_FILE="${RPM_DEB_INSTALLER_HOME}/systemDiagnostics.log"
}

rpmDeb_getUserInputs(){
    wrapper_getUserInputs
    if [[ "${PRODUCT_NAME}" == "${JFMC_LABEL}" || "${PRODUCT_NAME}" == "${INSIGHT_LABEL}" ]]; then
        rpmDeb_syncYaml "${KEY_ELASTICSEARCH_USERNAME}" "${SYS_KEY_SHARED_ELASTICSEARCH_USERNAME}" "${RPM_DEB_KEY_SYSTEM_TO_INSTALLER}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_USERNAME}"
        rpmDeb_syncYaml "${KEY_ELASTICSEARCH_PASSWORD}" "${SYS_KEY_SHARED_ELASTICSEARCH_PASSWORD}" "${RPM_DEB_KEY_SYSTEM_TO_INSTALLER}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_PASSWORD}"
    fi
...
rpmDeb_getUserInputsRabbitmq(){
    wrapper_getUserInputsRabbitmq
}

rpmDeb_hook_getUserInputs(){
    logDebug "rpmDeb_hook_getUserInputs (unimplemented)"
}

rpmDeb_ha_hook_getUserInputs(){
    logDebug "rpmDeb_ha_hook_getUserInputs (unimplemented)"
}

rpmDeb_hook_start_getUserInputs(){
    logDebug "rpmDeb_hook_start_getUserInputs (unimplemented)"
}

# Each product should try to discover data directory and set RPM_DEB_DATA_DIR with its value
# This will later be used to prompt user for this info or not
rpmDeb_hook_discoverDataDir(){
    logDebug "rpmDeb_hook_discoverDataDir (unimplemented)"
}

wrapper_hook_start_getUserInputs(){
    rpmDeb_hook_start_getUserInputs
    if [ -z "${RPM_DEB_DATA_DIR}" ]; then
...
rpmDeb_hook_setInstallerDefaults(){
    logDebug "rpmDeb_hook_setInstallerDefaults (unimplemented)"
}

wrapper_hook_setInstallerDefaults(){
    local dataDir=${RPM_DEB_DATA_DIR:-"${JF_PRODUCT_VAR_DEFAULT}"}
    wrapper_setSystemValue "${KEY_DATA_DIR_LOCATION}" "${dataDir}" "${FLAG_N}"
    rpmDeb_hook_setInstallerDefaults
}

rpmDeb_PostgresInfoToYaml(){
    local forceSync="${1}"
    if rpmDeb_isPostgresInstallEnabled; then
        rpmDeb_syncYaml "${KEY_DATABASE_PASSWORD}" "${SYS_KEY_SHARED_DATABASE_PASSWORD}" "${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}" || warn "Could not set ${SYS_KEY_SHARED_DATABASE_PASSWORD}"
        [[ ! -z "${forceSync}" && "${forceSync}" == "${FLAG_Y}" ]] && true || return 0
    fi

    local type="${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}"

    if [[ "${PRODUCT_NAME}" == "${JFMC_LABEL}" || "${PRODUCT_NAME}" == "${INSIGHT_LABEL}" || "${PRODUCT_NAME}" == "${DISTRIBUTION_LABEL}" || "${PRODUCT_NAME}" == "${XRAY_LABEL}" ]]; then
        rpmDeb_setSystemValueNoOverride "${SYS_KEY_SHARED_DATABASE_TYPE}"   "postgresql" "${JF_SYSTEM_YAML}"
        rpmDeb_setSystemValueNoOverride "${SYS_KEY_SHARED_DATABASE_DRIVER}" "org.postgresql.Driver" "${JF_SYSTEM_YAML}"
    fi
    rpmDeb_syncYaml "${KEY_DATABASE_URL}"      "${SYS_KEY_SHARED_DATABASE_URL}"      "${type}" || warn "Could not set ${SYS_KEY_SHARED_DATABASE_URL}"
    rpmDeb_syncYaml "${KEY_DATABASE_USERNAME}" "${SYS_KEY_SHARED_DATABASE_USERNAME}" "${type}" || warn "Could not set ${SYS_KEY_SHARED_DATABASE_USERNAME}"
    rpmDeb_syncYaml "${KEY_DATABASE_PASSWORD}" "${SYS_KEY_SHARED_DATABASE_PASSWORD}" "${type}" || warn "Could not set ${SYS_KEY_SHARED_DATABASE_PASSWORD}"
}

rpmDeb_RedisInfoToYaml(){
    local forceSync="${1}"
    local type="${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}"

    rpmDeb_syncYaml "${KEY_REDIS_PASSWORD}"      "${SYS_KEY_SHARED_REDIS_PASSWORD}"      "${type}" || warn "Could not set ${SYS_KEY_SHARED_REDIS_PASSWORD}"
}

rpmDeb_RabbitmqInfoToYaml(){
    local forceSync="${1}"
    local type="${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}"

    rpmDeb_syncYaml "${KEY_RABBITMQ_PASSWORD}"      "${SYS_KEY_SHARED_RABBITMQ_PASSWORD}"      "${type}" || warn "Could not set ${SYS_KEY_SHARED_RABBITMQ_PASSWORD}"
}

rpmDeb_ESInfoToYaml(){
    local type="${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}"

    rpmDeb_syncYaml "${SYS_KEY_SHARED_ELASTICSEARCH_CLUSTERSETUP}"   "${SYS_KEY_SHARED_ELASTICSEARCH_CLUSTERSETUP}"  "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_CLUSTERSETUP}"
    rpmDeb_syncYaml "${SYS_KEY_SHARED_ELASTICSEARCH_UNICASTFILE}"    "${SYS_KEY_SHARED_ELASTICSEARCH_UNICASTFILE}"   "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_UNICASTFILE}"
    rpmDeb_syncYaml "${KEY_ELASTICSEARCH_USERNAME}" "${SYS_KEY_SHARED_ELASTICSEARCH_USERNAME}" "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_USERNAME}"
    rpmDeb_syncYaml "${KEY_ELASTICSEARCH_PASSWORD}" "${SYS_KEY_SHARED_ELASTICSEARCH_PASSWORD}" "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_PASSWORD}"


    rpmDeb_syncYaml "${KEY_ELASTICSEARCH_URL}"      "${SYS_KEY_SHARED_ELASTICSEARCH_URL}"      "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_URL}"
    rpmDeb_syncYaml "${KEY_ELASTICSEARCH_USERNAME}" "${SYS_KEY_SHARED_ELASTICSEARCH_USERNAME}" "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_USERNAME}"
    rpmDeb_syncYaml "${KEY_ELASTICSEARCH_PASSWORD}" "${SYS_KEY_SHARED_ELASTICSEARCH_PASSWORD}" "${type}" || warn "Could not set ${SYS_KEY_SHARED_ELASTICSEARCH_PASSWORD}"
    if [[ "${PRODUCT_NAME}" == "${JFMC_LABEL}" || "${PRODUCT_NAME}" == "${INSIGHT_LABEL}" ]]; then
        rpmDeb_setSystemValueNoOverride "${SYS_KEY_SHARED_ELASTICSEARCH_EXTERNALISE}" "true" "${JF_SYSTEM_YAML}"
    fi
}
...
rpmDeb_syncYaml() {
    ...
    local keyToUpdate=${2}
    local valueToUpdate=""
    local type=${3:-"${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}"}
    
    local src="${4:-${INSTALLER_YAML}}"
    local target="${5:-${JF_SYSTEM_YAML}}"

    # Return if any of the inputs are empty
    [[ -z "$keyToFetch"    || "$keyToFetch"  == "" ]] && return 1
    [[ -z "$keyToUpdate"   || "$keyToUpdate"  == ""   ]] && return 1

    if [ "${type}" == "${RPM_DEB_KEY_SYSTEM_TO_INSTALLER}" ]; then
        src="${JF_SYSTEM_YAML}"
        target="${INSTALLER_YAML}"
    fi

    getSystemValue "${keyToFetch}" "NOT_SET" "false" "${src}"
    valueToUpdate="${YAML_VALUE}"

    if [ "${valueToUpdate}" == "NOT_SET" ]; then
        return 0
    fi

    if [[ "${target}" == "${JF_SYSTEM_YAML}" ]] || [[ "${target}" == "${INSTALLER_STATE_YAML_FILE}" ]]; then
        rpmDeb_setSystemValueNoOverride "${keyToUpdate}" "${valueToUpdate}" "${target}"
    else
        setSystemValue "${keyToUpdate}" "${valueToUpdate}" "${target}"
    fi
}

rpmDeb_setSystemValueNoOverride(){
    local key=$1
    local value=$2
    local yamlFile=$3
    ...
    if [ ${YAML_VALUE} == "NOT_SET" ]; then
        ...
        [ "${logMsg}" == "true" ] && echo "Skipping update of yaml key : ${key}" || true
    fi
}

rpmDeb_mandatoryInfoToYaml(){
    local type="${RPM_DEB_KEY_INSTALLER_TO_SYSTEM}"
    rpmDeb_syncYaml "${KEY_JFROGURL}"  "${SYS_KEY_SHARED_JFROGURL}"           "${type}" || warn "Could not set ${SYS_KEY_SHARED_JFROGURL}"    
    rpmDeb_syncYaml "${KEY_JOIN_KEY}"  "${SYS_KEY_SHARED_SECURITY_JOINKEY}"   "${type}" || warn "Could not set ${SYS_KEY_SHARED_SECURITY_JOINKEY}"
    rpmDeb_syncYaml "${KEY_HOST_IP}"   "${SYS_KEY_SHARED_NODE_IP}"            "${type}" || warn "Could not set ${SYS_KEY_SHARED_NODE_IP}"

    if [[ "${PRODUCT_NAME}" == "${PDN_SERVER_LABEL}" ]]; then
        rpmDeb_syncYaml "${KEY_PDNSERVER_SELFADDRESS}"   "${SYS_KEY_PDNSERVER_SELFADDRESS}"   "${type}" || warn "Could not set ${SYS_KEY_PDNSERVER_SELFADDRESS}"
    fi

    if [[ "${PRODUCT_NAME}" == "${PDN_NODE_LABEL}" ]]; then
        rpmDeb_syncYaml "${KEY_PDNNODE_PDNSERVERURL}"      "${SYS_KEY_PDNNODE_PDNSERVERURL}"      "${type}" || warn "Could not set ${SYS_KEY_PDNNODE_PDNSERVERURL}"
        rpmDeb_syncYaml "${KEY_PDNNODE_SELFGRPCADDRESS}"   "${SYS_KEY_PDNNODE_SELFGRPCADDRESS}"   "${type}" || warn "Could not set ${SYS_KEY_PDNNODE_SELFGRPCADDRESS}"
        rpmDeb_syncYaml "${KEY_PDNNODE_SELFHTTPADDRESS}"   "${SYS_KEY_PDNNODE_SELFHTTPADDRESS}"   "${type}" || warn "Could not set ${SYS_KEY_PDNNODE_SELFHTTPADDRESS}"
        rpmDeb_syncYaml "${KEY_PDNNODE_JOINKEY}"           "${SYS_KEY_PDNNODE_JOINKEY}"           "${type}" || warn "Could not set ${SYS_KEY_PDNNODE_JOINKEY}"
    fi
    createMasterkeyFile
}

rpmDeb_InstallerStateInfoToYaml(){
    local type="INSTALLER_TO_INSTALLER_STATE"
    ...
    for stateIndicatorKey in ${INSTALLER_STATE_KEYS}; do
        ...
        rpmDeb_syncYaml "${stateIndicatorValue}" "${stateIndicatorValue}" "${type}" "${INSTALLER_YAML}" "${INSTALLER_STATE_YAML_FILE}" \
            || warn "Could not set ${stateIndicatorValue}"
    done
}

createMasterkeyFile() {
    getSystemValue "${KEY_CLUSTER_MASTER_KEY}" "NOT_SET" "false" "${INSTALLER_YAML}"
    local masterKeyValue="${YAML_VALUE}"
    [[ -z "${masterKeyValue}" || "${masterKeyValue}" == "" || "${masterKeyValue}" == "NOT_SET" ]] && return
    masterKeyFile="$JF_PRODUCT_HOME/var/etc/security/master.key"
    createDir "${JF_PRODUCT_HOME}/var/etc/security" || warn "Could not create ${JF_PRODUCT_HOME}/var/etc/security directory"
    echo -n "${masterKeyValue}" > "${masterKeyFile}" || warn "Could not create ${masterKeyFile}"
}


rpmDeb_thirdPartyInfoToYaml(){
    rpmDeb_PostgresInfoToYaml
    rpmDeb_RedisInfoToYaml
    rpmDeb_RabbitmqInfoToYaml
}
...
rpmDeb_checkPrereqs(){
    ...
    rpmDeb_checkLinuxDistribution
    validationCheckPrerequisites
    rpmDeb_validateDiskSpace
    rpmDeb_validateSystemYaml
    rpmDeb_runSystemDiagnosticsChecks
    rpmDeb_checkUserEndInstalls
}

rpmDeb_checkPrereqsRabbitmq(){
    validationCheckUMask
    rpmDeb_checkLinuxDistribution
    validationCheckPrerequisites
    rpmDeb_validateDiskSpace
    mkdir -p "$JF_PRODUCT_HOME/var/etc"
    rpmDeb_validateSystemYaml
}

rpmDeb_setMaxOpenfiles() {
    if [[ "${PRODUCT_NAME}" == "${XRAY_LABEL}" ]]; then
        ...
        if [[ "$noFileVal" != "unlimited" ]] && [[ $noFileVal -lt $minNoFileMax ]]; then
            if [[ -f "/etc/security/limits.conf" ]]; then
                ...
                addLine "xray hard nofile 100000" "/etc/security/limits.conf"
                addLine "xray soft nofile 100000" "/etc/security/limits.conf"
            fi
...
rpmDeb_validateDiskSpace(){
    if [ ! -z "${JF_PRODUCT_HOME}" ]; then
        if [ -L "${JF_PRODUCT_HOME}/var" ]; then
            local targetLink=$(readlink -f "${JF_PRODUCT_HOME}/var")
            validateDiskSpace "${targetLink}"
        else
            [ -d "${JF_PRODUCT_HOME}/var" ] && validateDiskSpace "${JF_PRODUCT_HOME}/var"
        fi
    fi
}
moveSystemYaml(){
    if [ -f "${JF_SYSTEM_YAML}" ]; then
        local jfrogUrl=""
        local databaseUrl=""

        getSystemValue "${SYS_KEY_SHARED_JFROGURL}" "NOT_SET" "false" "${JF_SYSTEM_YAML}"
        jfrogUrl="${YAML_VALUE}"

        getSystemValue "${SYS_KEY_SHARED_DATABASE_URL}" "NOT_SET" "false" "${JF_SYSTEM_YAML}"
        databaseUrl="${YAML_VALUE}"

        getSystemValue "${SYS_KEY_PDNNODE_PDNSERVERURL}" "NOT_SET" "false" "${JF_SYSTEM_YAML}"
        pdnNodeServerUrl="${YAML_VALUE}"

        # Do not rename system.yaml if,
        #           - jfrog url is already set
        #           - database url is set
        # This will make sure, 
        #       - We are writing to a good content of system.yaml and it already modified atleast once
        #       - There are no empty objects which will result in yq updating it to null
        [ "${jfrogUrl}"    == "NOT_SET" ] || return 0
        [ "${databaseUrl}" == "NOT_SET" ] || return 0
        [ "${pdnNodeServerUrl}"  == "NOT_SET" ] || return 0

        local backupSystemYaml="${JF_SYSTEM_YAML}.$(date +"%Y%m%d%H%M")"
        logDebug "Found a system yaml ${JF_SYSTEM_YAML}, moving this to ${backupSystemYaml}"
        mv "${JF_SYSTEM_YAML}" "${backupSystemYaml}" >&6 2>&1
    fi
}

rpmDeb_hook_preInstall(){
    logDebug "rpmDeb_hook_preInstall (unimplemented)"
}

rpmDeb_hook_postInstall(){
    logDebug "rpmDeb_hook_postInstall (unimplemented)"
}

rpmDeb_hook_writeInfoToYaml(){
    logDebug "rpmDeb_hook_writeInfoToYaml (unimplemented)"
}

rpmDeb_hook_preUserInputs(){
    logSilly "Method ${FUNCNAME[0]}"

}

rpmDeb_pdnConsoleLoggingTosystemYaml(){
    if [[ "${PRODUCT_NAME}" == "${PDN_NODE_LABEL}" || "${PRODUCT_NAME}" == "${PDN_SERVER_LABEL}" ]]; then
        rpmDeb_setSystemValueNoOverride "${SYS_KEY_SHARED_LOGGING_CONSOLELOG_ENABLED}" "true" "${JF_SYSTEM_YAML}"
    fi
}

rpmDeb_writeInfoToYaml(){
    moveSystemYaml
    rpmDeb_thirdPartyInfoToYaml
    rpmDeb_mandatoryInfoToYaml
    rpmDeb_hook_writeInfoToYaml
    rpmDeb_InstallerStateInfoToYaml
    if [[ "${INSTALLATION_METHOD}" == "xray" ]]; then
        setSystemValue "installation_method" "xray" "$INSTALLER_STATE_YAML_FILE"
    else
        setSystemValue "installation_method" "all" "$INSTALLER_STATE_YAML_FILE"
    fi
}

createSystemYaml(){
     if [ -f "${JF_SYSTEM_YAML}" ]; then
        logDebug "Found a system yaml ${JF_SYSTEM_YAML}"
     else
        mkdir -p "$JF_PRODUCT_HOME/var/etc"
        touch "${JF_SYSTEM_YAML}"
        chown -R ${JF_XRAY_USER}:${JF_XRAY_GROUP} "$JF_PRODUCT_HOME/var/etc"
     fi
}

rpmDeb_writeInfoToYamlRabbitmq(){
    createSystemYaml
    rpmDeb_RabbitmqInfoToYaml
    rpmDeb_hook_writeInfoToYaml
    rpmDeb_InstallerStateInfoToYaml
    setSystemValue "shared.rabbitMq.autoStop" "true" "$JF_SYSTEM_YAML"
    setSystemValue "installation_method" "rabbitmq" "$INSTALLER_STATE_YAML_FILE"
}

rpmDeb_validateSystemYaml() {
    wrapper_validateSystemYaml "${JF_PRODUCT_HOME}/var/etc/${FILE_NAME_SYSTEM_YAML}" "${JF_PRODUCT_HOME}/app/misc/etc/${FILE_NAME_SYSTEM_YAML_TEMPLATE}" "${SCRIPT_HOME}/diagnostics/diagnosticsUtil"
}

rpmDeb_runSystemDiagnosticsChecks() {
    runSystemDiagnosticsChecks "${JF_PRODUCT_HOME}" "${SCRIPT_HOME}/diagnostics/diagnosticsUtil" "${SCRIPT_HOME}/diagnostics/diagnostics.yaml"
}

rpmDeb_cleanup(){
    [ -f "${INSTALLER_YAML}" ] && rm -f "${INSTALLER_YAML}" >/dev/null 2>&1 || true
}

removeOldServices(){
    local oldServices=$1
    local installScript=${JF_PRODUCT_HOME}/app/bin/uninstallService.sh

    for service in ${oldServices}; do
        service ${service} stop >/dev/null 2>&1;
        ${JF_PRODUCT_HOME}/app/bin/uninstallService.sh $service >/dev/null 2>&1;
    done
}

deployInitScriptsRabbitmq() {
    local xrayServices="rabbitmq"
    local installScript=${JF_PRODUCT_HOME}/app/bin/installService.sh

    if [ ! -f ${installScript} ]; then
      echo "ERROR : Unable to find ${installScript}, this is used to construct and copy init script for each microservice"
      exit 1
    fi

    for service in ${xrayServices}; do
        ${JF_PRODUCT_HOME}/app/bin/installService.sh $service
    done
}

setUserGroupRabbitmq(){
    local user=
    local group=

    getSystemValue "xray.user" "NOT_SET"
    user=$YAML_VALUE

    getSystemValue "xray.group" "NOT_SET"
    group=$YAML_VALUE

    # Consider xray user as group if its ony JF_XRAY_USER is set
    if [[ "$group" == "NOT_SET" && "$user" != "NOT_SET" ]]; then
        group="$user"
    fi
    ...
    # Default it to xray
    JF_XRAY_USER=${user:-xray}
    JF_XRAY_GROUP=${group:-xray}
}

# Do the actual permission check and chown
checkAndSetOwnerOnDirRabbitmq () {
    local dirToCheck=$1
    local userToCheck=$2
    local groupToCheck=$3
...
setVarLinkOwnershipRabbitmq(){
    ...
    if [[ "${curTargetVar}" == *"${varLink}"* ]]; then
        logger "Creating a symlink from ${varLink} pointing to directory ${newTargetVar}"
        mkdir -p "${newTargetVar}"                                   || errorExit "Create directory failed, command : mkdir -p \"${newTargetVar}\""
        checkAndSetOwnerOnDirRabbitmq "${newTargetVar}" "${user}" "${group}"
        ln -s "${newTargetVar}" "${varLink}"                         || errorExit "Failed to link data directory to JF_PRODUCT_HOME/var, command : ln -s \"${newTargetVar}\" \"${varLink}\""
    else
        # If custom data is not set, use link that is read to create and change permission of target directory
        [[ "${isCustomDataSet}" == "no" ]] && targetVar="${curTargetVar}" || targetVar="${newTargetVar}"
        ...
        mkdir -p "${targetVar}"                || errorExit "Create directory failed, command : mkdir -p \"${targetVar}\""
        checkAndSetOwnerOnDirRabbitmq "${targetVar}" "${user}" "${group}"
...
createUserGroupRabbitmq() {
    JF_PRODUCT_DATA_DEFAULT="/var/opt/jfrog/xray"
    JF_PRODUCT_DATA_INTERNAL="${JF_PRODUCT_VAR:-${JF_PRODUCT_DATA_DEFAULT}}"
    setUserGroupRabbitmq

    getent group "${JF_XRAY_GROUP}" 1>/dev/null 2>&1
    if [ $? != 0  ]; then
      echo "Group $JF_XRAY_GROUP doesn't exist. Creating ..."
      /usr/sbin/groupadd -r ${JF_XRAY_GROUP} 1>/dev/null 2>&1 || exit $?
    else
      echo "Group $JF_XRAY_GROUP already exists"
    fi

    getent passwd "${JF_XRAY_USER}" 1>/dev/null 2>&1
    if [ $? != 0 ]; then
      echo "User $JF_XRAY_USER doesn't exist. Creating ..."
      /usr/sbin/useradd -r -d "${JF_PRODUCT_HOME}" -s /bin/bash -m -g "${JF_XRAY_GROUP}" "${JF_XRAY_USER}" 1>/dev/null 2>&1 || exit $?
      chown ${JF_XRAY_USER}:${JF_XRAY_GROUP} "${JF_PRODUCT_HOME}"
    else
      echo "User $JF_XRAY_USER already exists"
    fi

    if [ ! -L "${JF_PRODUCT_HOME}/var" ]; then
      createDir "${JF_PRODUCT_DATA_INTERNAL}" "${JF_XRAY_USER}" "${JF_XRAY_GROUP}"
      ln -s "${JF_PRODUCT_DATA_INTERNAL}" "${JF_PRODUCT_HOME}/var" || { echo "ERROR : Failed to create symlink from ${JF_PRODUCT_DATA_INTERNAL} to ${JF_PRODUCT_HOME}/var"; exit 1; }
      chown ${JF_XRAY_USER}:${JF_XRAY_GROUP} "${JF_PRODUCT_HOME}/var" "${JF_PRODUCT_DATA_INTERNAL}"
    fi

    setVarLinkOwnershipRabbitmq "${JF_XRAY_USER}" "${JF_XRAY_GROUP}" "${JF_PRODUCT_HOME}/var" "${JF_PRODUCT_DATA_DEFAULT}"
    
    export LC_ALL=C
}

deployRabbitmqService(){
    removeOldServices "xray-rabbitmq xray xray-server xray-router xray-indexer xray-observability xray-persist xray-analysis"
    deployInitScriptsRabbitmq
}
...
rpmDeb_main() {
    ...
    if [[ "${INSTALLATION_METHOD}" == "rabbitmq" ]]; then
        bannerStart "Beginning JFrog ${PRODUCT_NAME} rabbitmq setup"
        rpmDeb_init
        rpmDeb_setYQPath
        rpmDeb_createLogFile
        rpmDeb_startLogRabbit
        rpmDeb_setMaxOpenfiles
        rpmDeb_checkPrereqsRabbitmq
        rpmDeb_isUpgrade "${RPM_DEB_PRODUCT_PACK_NAME}"
        rpmDeb_hook_discoverDataDir
        rpmDeb_getUserInputsRabbitmq
        rpmDeb_hook_preInstall
        # INSTALLATION_METHOD is used here to determine to install rpm with no scripts options (to not install the xray services)
        rpmDeb_installProduct
        createUserGroupRabbitmq
        rpmDeb_writeInfoToYamlRabbitmq
        rpmDeb_hook_postInstall
        ...
        # Remove the standalone rabbitmqmq service if present. 
        removeOldServices "xray-rabbitmq"
...
linux_init(){
    ...
    RPM_DEB_THIRDPARTY_HOME="${JF_PRODUCT_HOME}/app/third-party"
    LINUX_THIRDPARTY_HOME="${JF_PRODUCT_HOME}/app/third-party"
    LINUX_POSTGRES_BINARIES="${JF_PRODUCT_HOME}/app/third-party/pgsql"
    INSTALLATION_LOG_FILE="${JF_PRODUCT_HOME}/var/log/install.log"
    SYSTEM_DIAGNOSTICS_LOG_FILE="${JF_PRODUCT_HOME}/var/log/systemDiagnostics.log"

    JF_PRODUCT_VAR_DEFAULT=${JF_PRODUCT_HOME}/var
    RPM_DEB_DEFAULT_ENV=${JF_PRODUCT_HOME}/app/bin/${RPM_DEB_PRODUCT_PACK_NAME}.default

    INSTALLER_YAML=${SCRIPT_HOME}/${FILE_NAME_INSTALLER_YAML}
    RPM_DEB_SYSTEM_YAML=${JF_PRODUCT_HOME}/var/etc/system.yaml
    RPM_DEB_DATA_DIR=${JF_PRODUCT_HOME}/var

    PRODUCT_USER=${PRODUCT_USER:-"${DEFAULT_PRODUCT_USER}"}
    PRODUCT_GROUP=${PRODUCT_GROUP:-"${PRODUCT_USER}"}

    export JF_INSTALL_ELASTICSEARCH="$FLAG_N"
...
linux_createLogFile(){
    redirectLogsToFile #creates variable DESTINATION_LOG_FILE
    INSTALLATION_LOG_FILE="${JF_PRODUCT_HOME}/var/log/install.log"
    createDir "$(dirname ${INSTALLATION_LOG_FILE})" "${PRODUCT_USER}" "${PRODUCT_GROUP}"
    createDir "$(dirname ${SYSTEM_DIAGNOSTICS_LOG_FILE})" "${PRODUCT_USER}" "${PRODUCT_GROUP}"
    configureLogOutput
}

linux_skipPostgresSetup(){
    if [[ ! -z "${SKIP_POSTGRES_SETUP}" && "${SKIP_POSTGRES_SETUP}" == "${FLAG_Y}" ]]; then
        return 0;
    else
        return 1;
...
linux_warnPostgresSetupFailure(){
    local warn="${1}"
    warn "${1}"
    logger "Please refer ${LINUX_USER_END_INSTALL_DOC} for instructions to set up PostgreSQL"
}

linux_isPostgresRunning(){
    if ps -ef | grep "pg_ctl" | grep -v grep >/dev/null 2>&1; then
        return 0;
    else
        return 1;
    fi
...
linux_installPostgres(){
    if linux_isPostgresRunning ; then
        linux_warnPostgresSetupFailure "Detected a running PosgreSQL process running with pg_ctl, skipping its automatic installation"
        return 0;
    fi

    if [ -d "${LINUX_POSTGRES_DATA}" ]; then
        linux_warnPostgresSetupFailure "Detected a PosgreSQL data directory on this machine, skipping its automatic installation"
        return 0;
    fi

    local pgCtrlCmd="${LINUX_POSTGRES_BINARIES}/bin/pg_ctl -D ${LINUX_POSTGRES_DATA}"
    local CMD=""
    local cmdPrefix="LC_ALL=en_US.UTF-8 LC_CTYPE=en_US.UTF-8"
    local startCMD="${cmdPrefix} ${pgCtrlCmd} -l ${LINUX_POSTGRES_DATA}/postgres-service.log start"
    local stopCMD="${cmdPrefix} ${pgCtrlCmd} -l ${LINUX_POSTGRES_DATA}/postgres-service.log stop"
    local statusCMD="${cmdPrefix} ${pgCtrlCmd} -l ${LINUX_POSTGRES_DATA}/postgres-service.log status"
    local suCmdPrefix="su - -c"

    mkdir -p "${LINUX_POSTGRES_DATA}" || \
        { linux_warnPostgresSetupFailure "Failed to create directory, [ mkdir -p ${LINUX_POSTGRES_DATA} ]"; return 0; }
    chown -R ${PRODUCT_USER} "${LINUX_POSTGRES_DATA}" || \
        { linux_warnPostgresSetupFailure "Failed to change ownership of directory, [ chown -R ${PRODUCT_USER} ${LINUX_POSTGRES_DATA} ]"; return 0; }

    # Initialize the data directory
    logger "Initialising postgresql data directory - ${LINUX_POSTGRES_DATA}"
    CMD="${cmdPrefix} ${LINUX_POSTGRES_BINARIES}/bin/initdb -D ${LINUX_POSTGRES_DATA}"    
    ${suCmdPrefix} "$CMD" ${PRODUCT_USER} >&6 2>&1 || \
        { linux_warnPostgresSetupFailure "Failed to initialise postgres data directory, [ ${suCmdPrefix} '$CMD' ${PRODUCT_USER} ]"; return 0; }

    sleep 2

    # Start Postgresql
    logger "Starting postgresql"
    ${suCmdPrefix} "$startCMD" ${PRODUCT_USER} >&6 || \
        { linux_warnPostgresSetupFailure "Failed to start postgres, [ ${suCmdPrefix} '$startCMD' ${PRODUCT_USER} ]"; return 0; }
    
    # Enabling Postgres connectivity from remote servers, Will be needed only for HA or externalized database installations
    addLine "host    all             all             0.0.0.0/0               md5" "${LINUX_POSTGRES_DATA}/pg_hba.conf"
    
    # Add the following line to ${LINUX_POSTGRES_DATA}/postgresql.conf
    addLine "listen_addresses='*'" "${LINUX_POSTGRES_DATA}/postgresql.conf"
    
    sleep 2

    # Restart Postgres process
    logger "Changing postgreql conf and restarting postgresql"
    ${suCmdPrefix} "$stopCMD" ${PRODUCT_USER} >&6 || \
        { linux_warnPostgresSetupFailure "Failed to start postgres, [ ${suCmdPrefix} '$stopCMD' ${PRODUCT_USER} ]"; return 0; }
    ${suCmdPrefix} "$startCMD" ${PRODUCT_USER} >&6 || \
        { linux_warnPostgresSetupFailure "Failed to start postgres, [ ${suCmdPrefix} '$startCMD' ${PRODUCT_USER} ]"; return 0; }
    
    sleep 5

    # Create the psql database (the script "${LINUX_POSTGRES_BINARIES}/createPostgresUsers.sh" , responsible for seeding Postgres assumes this database exists)
    logger "Creating ${PRODUCT_USER} database with default template"
    CMD="${cmdPrefix} ${LINUX_POSTGRES_BINARIES}/bin/psql -d template1 --host=localhost --port=5432 -c 'CREATE DATABASE ${PRODUCT_USER};'"
    ${suCmdPrefix} "$CMD" ${PRODUCT_USER} >&6 || \
        { linux_warnPostgresSetupFailure "Failed to create user template in postgres, [ ${suCmdPrefix} '$CMD' ${PRODUCT_USER} ]"; return 0; }
    
    sleep 2

    logger "Creating postgresql database, user and schema required by product"
    CMD="${cmdPrefix} POSTGRES_PATH=${LINUX_POSTGRES_BINARIES}/bin DB_PASSWORD=${RPM_DEB_POSTGRES_PASSWORD} ${LINUX_THIRDPARTY_HOME}/postgresql/createPostgresUsers.sh"
    ${suCmdPrefix} "$CMD" ${PRODUCT_USER} >&6 || \
        { linux_warnPostgresSetupFailure "Failed to create postgresql database and users, [ ${suCmdPrefix} '$CMD' ${PRODUCT_USER} ]"; return 0; }

    cat << POSTGRESQL_MANAGE_COMMANDS

PostgreSQL can be managed with following commands,

    Start   -   ${suCmdPrefix} '$startCMD' ${PRODUCT_USER}
    Stop    -   ${suCmdPrefix} '$stopCMD' ${PRODUCT_USER}
    Status  -   ${suCmdPrefix} '$statusCMD' ${PRODUCT_USER}

POSTGRESQL_MANAGE_COMMANDS
}

linux_postgresInit(){
    LINUX_DEFAULT_POSTGRES_DATA="/usr/local/pgsql/data"
    LINUX_POSTGRES_DATA=${LINUX_POSTGRES_DATA:-${LINUX_DEFAULT_POSTGRES_DATA}}
}

installPostgres() {
    if linux_skipPostgresSetup || ! rpmDeb_isPostgresInstallEnabled; then
        return 0;
    fi
    ...
    linux_postgresInit
    linux_installPostgres
    linux_writePostgresDefaultDetails
}

linux_productStartInfo(){
    local serviceName="$1"
    local productName="${2:-product}"
...
NOTE : Make sure ${JF_PRODUCT_HOME} is owned by ${PRODUCT_USER}.
    chown -R ${PRODUCT_USER}:${PRODUCT_GROUP} ${JF_PRODUCT_HOME}

To start ${productName}:
(1) login as ${PRODUCT_USER} user
(2) ${JF_PRODUCT_HOME}/app/bin/${serviceName}.sh start

To check ${productName} status:
(1) login as ${PRODUCT_USER} user
(2) ${JF_PRODUCT_HOME}/app/bin/${serviceName}.sh status
...
linux_endInfo() {
    ...
    linux_productStartInfo "${LINUX_PRODUCT_SERVICE_NAME}" "${PRODUCT_NAME}"
...
linux_hook_start_getUserInputs(){
    logSilly "Method ${FUNCNAME[0]}"
}
...
linux_checkPrerequisitesSoftware() {
    # Validate that any pre-requisites specified by the 3rd party libraries are present
    local userEndInstalls="$1"
    local depencenciesURL="$2"
    local packDisplayName=""
    local packName=""
    if [[ -z "${userEndInstalls}" || -z "${depencenciesURL}" ]]; then
        return 0
    fi

    local unavailableTools=""
    local toolList=($userEndInstalls)

    for packKeyValue in "${toolList[@]}"
    do
        packName=$(echo "${packKeyValue}" | awk -F"=" '{print $1}')
        packDisplayName=$(echo "${packKeyValue}" | awk -F"=" '{print $2}')

        [ -z "${packDisplayName}" ] && packDisplayName="${packName}" || true

        if [[ "$(io_commandExists ${packName})" != "yes" ]]; then
          unavailableTools="${unavailableTools} ${packDisplayName}"
        fi
    done

    if [ ! -z "${unavailableTools}" ]; then
        logError "Some dependencies [ ${unavailableTools} ] the product needs are missing"
        errorExit "Please check Prerequisites section under ${depencenciesURL} on instructions to install them and then try again"
    fi
}

linux_writePostgresDefaultDetails() {
    local prefix="${1:-LINUX}"
    local sysKey=""
    local defaultKey=""

    for databaseKey in DATABASE_URL DATABASE_USERNAME DATABASE_PASSWORD; do

        _updateFromSystemYaml "SHARED_${databaseKey}" "KEY_${databaseKey}"

        sysKey="SYS_KEY_SHARED_${databaseKey}"
        defaultKey="${prefix}_POSTGRES_${databaseKey}"

        [ -z "$YAML_VALUE" ] && \
        rpmDeb_setSystemValueNoOverride "${!sysKey}" "${!defaultKey}" "${INSTALLER_YAML}" || true
    done
}

rpmDeb_thirdPartyInfoToYaml(){
    rpmDeb_PostgresInfoToYaml "${FLAG_Y}"
    rpmDeb_ESInfoToYaml "${FLAG_Y}"
    rpmDeb_RedisInfoToYaml "${FLAG_Y}"
    rpmDeb_RabbitmqInfoToYaml "${FLAG_Y}"
}

linux_preUserInputChecks(){
    wrapper_setSystemValue "$KEY_POSTGRES_INSTALL" "$FLAG_N" "${overwrite}"
    SKIP_POSTGRES_SETUP=${FLAG_Y}
}

linux_askPostgresdetails(){
    getYamlValue "$KEY_POSTGRES_INSTALL" "$INSTALLER_YAML" "false"
    isPostgresExternalised=$([ "$YAML_VALUE" == "$FLAG_Y" ] && echo -n "$FLAG_N" || echo -n "$FLAG_Y")
...
linux_setOwnership(){
    for item in ${JF_SYSTEM_YAML} ${INSTALLER_STATE_YAML_FILE} ${INSTALLATION_LOG_FILE} ; do
        io_setOwnership "${item}" "${PRODUCT_USER}" "${PRODUCT_GROUP}"
    done
}
...
-h | --help                                       : [optional] display usage
-u | --user                                       : [optional] user which will be used to run the product, it will be created if its unavailable
-g | --group                                      : [optional] group which will be used to run the product, it will be created if its unavailable
--pg-data                                         : [optional] (default: /usr/local/pgsql/data) pass the data directory for postgres installation 
END_USAGE
    sleep .5
}

linux_processOptions() {
    VERBOSE_MODE=false
...
linux_installService_processOptions() {
    VERBOSE_MODE=false
...
linux_setProductHome() {
    local defaultEnv=$1

    if [[ -z "${JF_PRODUCT_HOME}" ]] || [[ -z "${defaultEnv}" ]]; then
        return 0;
    fi

    addOrReplaceProperty "export JF_PRODUCT_HOME" "${JF_PRODUCT_HOME}" "${defaultEnv}" \
        || errorExit "Failed to set JF_PRODUCT_HOME in ${defaultEnv}"
}

linux_createRunDir() {
    local runDir="$JF_PRODUCT_HOME/app/run"
    if [ ! -d "${runDir}" ]; then
        mkdir -p "${runDir}" || errorExit "Failed to create ${runDir}"
    fi
}

linux_stopService() {
    local serviceName="$1"
    [ ! -z "${serviceName}" ] || return 0
    local initFile="/etc/init.d/${serviceName}"

    # Shutting down the product service if running
    if [ -e "${initFile}" ]; then
        logger "Stopping the ${serviceName} service"
        ${initFile} stop >/dev/null 2>&1
        [ -x /bin/systemctl ] && /bin/systemctl stop "${LINUX_SERVICE_NAME}" >/dev/null 2>&1 || true
    fi
}

linux_installService() {
    linux_uninstallService >/dev/null
    if [[ "${IS_SYSTEMD_ENABLED}" != "${FLAG_Y}" ]]; then
        linux_installInitdService
        return;
    fi

    if systemctl -h > /dev/null 2>&1; then
        # Installing service from type systemd
        linux_installSystemdService
    else
...
linux_uninstallService() {
    linux_stopService "${LINUX_SERVICE_NAME}"
    linux_uninstallInitdService
}

linux_uninstallInitdService(){
    logger "Removing $LINUX_SERVICE_NAME service from auto-start"
    local removeOk=
    if [ -x /bin/systemctl ]; then
        /bin/systemctl disable "${LINUX_SERVICE_NAME}" >/dev/null 2>&1 && removeOk=true
        local SUSE=$(cat /etc/issue | grep -io suse)
        local NOSUSE=$(echo $SUSE | tr "[:upper:]" "[:lower:]")
        local UNAME=$(uname | tr "[:upper:]" "[:lower:]")    
        local unitFile=

        if [ "$UNAME" == "linux" ] && [ "$NOSUSE" != "suse" ]; then
            unitFile="/lib/systemd/system/${LINUX_SERVICE_NAME}.service"
        else
            unitFile="/usr/lib/systemd/system/${LINUX_SERVICE_NAME}.service"
        fi
        rm -f "$unitFile" >/dev/null 2>&1;
        /bin/systemctl daemon-reload >/dev/null 2>&1;
        /bin/systemctl reset-failed "${LINUX_SERVICE_NAME}" >/dev/null 2>&1;
    fi

    if [ -x /usr/sbin/update-rc.d ]; then
        /usr/sbin/update-rc.d -f "${LINUX_SERVICE_NAME}" remove >/dev/null 2>&1 && removeOk=true
    elif [ -x /usr/sbin/chkconfig ]; then
        /usr/sbin/chkconfig --del "${LINUX_SERVICE_NAME}" >/dev/null 2>&1 && removeOk=true
    elif [ -x /sbin/chkconfig ]; then
        /sbin/chkconfig --del "${LINUX_SERVICE_NAME}" >/dev/null 2>&1 && removeOk=true
    fi

    [ ${removeOk} ] || warn "Uninstalling service ${LINUX_SERVICE_NAME} failed"

    removeOk=
    if [ -e "${LINUX_INIT_SERVICE_FILE}" ]; then
        rm -f ${LINUX_INIT_SERVICE_FILE} >/dev/null 2>&1 && removeOk=true
        [ -e ${LINUX_INIT_SERVICE_FILE}.init.backup ] && rm -f ${LINUX_INIT_SERVICE_FILE}.init.backup >/dev/null 2>&1 || true
    else
        removeOk=true
    fi

    [ ${removeOk} ] || warn "Removing $LINUX_INIT_SERVICE_FILE failed, can be removed manually"
}

linux_installSystemdService() {
    SYSTEMD_UNIT_TEMPLATE=${JF_PRODUCT_HOME}/app/misc/service/${LINUX_SERVICE_NAME}-systemd.service

    SUSE=$(cat /etc/issue | grep -io suse)
    NOSUSE=$(echo $SUSE | tr "[:upper:]" "[:lower:]")
    UNAME=$(uname | tr "[:upper:]" "[:lower:]")
    if [ "$UNAME" == "linux" ] && [ "$NOSUSE" != "suse" ]; then
        SYSTEMD_UNIT_FILE="/lib/systemd/system/${LINUX_SERVICE_NAME}.service"
    else
        SYSTEMD_UNIT_FILE="/usr/lib/systemd/system/${LINUX_SERVICE_NAME}.service"
    fi

    local initFile=/etc/init.d/${LINUX_SERVICE_NAME}
    if [ -e ${initFile} ]; then
        mv ${initFile} ${initFile}.disabled
    fi

    cp -f ${SYSTEMD_UNIT_TEMPLATE} ${SYSTEMD_UNIT_FILE}
    chmod 644 ${SYSTEMD_UNIT_FILE}

    sed -i -e "s,__JF_PRODUCT_HOME__,$JF_PRODUCT_HOME,g" ${SYSTEMD_UNIT_FILE}

    # Use the systemctl command to enable the service
    echo "Initializing $LINUX_SERVICE_NAME service with systemctl..."
    systemctl daemon-reload &>/dev/null
    systemctl enable ${LINUX_SERVICE_NAME} &>/dev/null
    systemctl list-unit-files --type=service | grep ${LINUX_SERVICE_NAME}.service &>/dev/null || errorExit "Could not install ${LINUX_SERVICE_NAME} service"
}

linux_installInitdService() {
    [ ! -z "${LINUX_INIT_SERVICE_FILE}" ]     || errorExit "LINUX_INIT_SERVICE_FILE is not set, could not install service"
    [ ! -z "${LINUX_INIT_SERVICE_TEMPLATE}" ] || errorExit "LINUX_INIT_SERVICE_TEMPLATE is not set, could not install service"
    [ ! -z "${LINUX_SERVICE_NAME}" ]          || errorExit "LINUX_SERVICE_NAME is not set, could not install service"
    [ ! -z "${JF_PRODUCT_HOME}" ]             || errorExit "JF_PRODUCT_HOME is not set, could not install service"

    local chkconfigOK=

    if [ -e "$LINUX_INIT_SERVICE_FILE" ]; then
        cp -f ${LINUX_INIT_SERVICE_FILE} ${LINUX_INIT_SERVICE_FILE}.init.backup >/dev/null 2>&1
    fi

    logger "Installing ${LINUX_SERVICE_NAME} as a service"
    
    cp -f ${LINUX_INIT_SERVICE_TEMPLATE} ${LINUX_INIT_SERVICE_FILE} \
        || errorExit "Failed to copy init script template, command [ cp -f ${LINUX_INIT_SERVICE_FILE} ${LINUX_INIT_SERVICE_FILE}.init.backup ]"

    chmod a+x ${LINUX_INIT_SERVICE_FILE} \
        || errorExit "Failed to make init file executable, command [ chmod a+x ${LINUX_INIT_SERVICE_FILE} ]"

    sed -i -e "
            s,__JF_PRODUCT_HOME__,${JF_PRODUCT_HOME},g;
            s,__SERVICE__,$LINUX_SERVICE_NAME,g" ${LINUX_INIT_SERVICE_FILE} \
        || errorExit "Failed to home and service name in ${LINUX_INIT_SERVICE_FILE} script"

    # Try update-rc.d for debian/ubuntu else use chkconfig
    if [ -x /usr/sbin/update-rc.d ]; then
        update-rc.d ${LINUX_SERVICE_NAME} defaults &>/dev/null && \
        chkconfigOK=true
    elif [ -x /usr/sbin/chkconfig ] || [ -x /sbin/chkconfig ]; then
        chkconfig --add ${LINUX_SERVICE_NAME} &>/dev/null && \
        chkconfig ${LINUX_SERVICE_NAME} on &>/dev/null && \
        chkconfig --list ${LINUX_SERVICE_NAME} &>/dev/null && \
        chkconfigOK=true
    else
        ln -s ${LINUX_INIT_SERVICE_FILE} /etc/rc3.d/S99${LINUX_SERVICE_NAME} && \
        chkconfigOK=true
    fi
    [ ${chkconfigOK} ] || errorExit "Service ${LINUX_SERVICE_NAME} installation failed"
}

linux_writeUserInfo(){
    rpmDeb_setSystemValueNoOverride ${LINUX_USER_KEY} "${PRODUCT_USER}" "${JF_SYSTEM_YAML}" || true
    rpmDeb_setSystemValueNoOverride ${LINUX_GROUP_KEY} "${PRODUCT_GROUP}" "${JF_SYSTEM_YAML}" || true
}

linux_setownership(){
    io_setOwnership "${JF_PRODUCT_HOME}/app" "${PRODUCT_USER}" "${PRODUCT_GROUP}"
    io_setOwnership "${JF_PRODUCT_HOME}/var/etc" "${PRODUCT_USER}" "${PRODUCT_GROUP}"
}
...
linux_installService_main(){
    ...
    setUserGroup "${LINUX_USER_KEY}" "${LINUX_GROUP_KEY}" "${PRODUCT_USER}" "${PRODUCT_GROUP}"

    createUserAndGroup

    linux_setProductHome "${LINUX_SERVICE_DEFAULT_ENV}"
    ...
    displayRpmDebStartInfo "${LINUX_SERVICE_NAME}" "${PRODUCT_NAME}"
}
...
linux_uninstallService_main(){
    ...
    linux_displayPostUninstallInfo "${LINUX_SERVICE_NAME}" "${PRODUCT_NAME}"    
}

linux_displayPostUninstallInfo(){
    logger "Uninstallation of service ${LINUX_SERVICE_NAME} completed"
}
...
linux_main() {
    ...
    linux_checkPrerequisitesSoftware "${LINUX_USER_END_INSTALL_LIST}" "${LINUX_USER_END_INSTALL_DOC}"
