...
getXrayVersion () {
    ...
    local newfilePath="$3"
    local oldfilePath="$4"
    local propertyInDocker="XRAY_VERSION"
    local productVersion=
    local status=
    ...
    if [[ "$INSTALLER" == "${COMPOSE_TYPE}" ]]; then
        if [[ -f "${oldfilePath}" ]]; then
            ...
            logger "File [${oldfilePath}] or [${newfilePath}] not found to get current version. Skipping migration"
            exit 0
        fi
    elif [[ "$INSTALLER" == "${ZIP_TYPE}" ]]; then
        if [[ -f "${newfilePath}" ]]; then
            productVersion="$(cat ${newfilePath} | grep xray | awk 'NR==1{print $2}')"
            status="fail"
        fi
    else
        if [[ -f "${oldfilePath}" ]]; then
            local xrayVersion="XRAY_VERSION"
            unset ${xrayVersion}
            source ${oldfilePath}
            productVersion="$(evalVariable "${xrayVersion}" "${xrayVersion}")"
            status="success"
        elif [[ -f "${newfilePath}" ]]; then
            productVersion="$(cat ${newfilePath} | grep xray | awk 'NR==1{print $2}')"
            status="fail"
    ...
    if [[ -z "${productVersion}" || "${productVersion}" == "" ]]; then
        [[ "${status}" == "success" ]] && logger "No version found in file [${oldfilePath}]. Skipping migration"
        [[ "${status}" == "fail" ]] && logger "No version found in file [${newfilePath}]. Skipping migration"
        exit 0
    fi
...
changeOwnership_hook () {
    local thirdpartyDir="$1"
    local user_id="$2"
    if [[ "$(checkDirExists "${thirdpartyDir}")" == "true" ]]; then
        chown -R ${user_id}:${user_id} "${thirdpartyDir}" && status=success || status=fail
        [[ "${status}" == "success" ]] && logger "Successfully changed ownership of directory [${thirdpartyDir}] to ${user_id}:${user_id}"
        [[ "${status}" == "fail" ]] && warn "Failed to change ownership of directory [${thirdpartyDir}] to ${user_id}"
    else
        warn "Directory ${thirdpartyDir} does not exist to change ownership"
    fi
}

changeThirdpartyDirOwnership () {
    local rabbitmqDir="${NEW_DATA_DIR}/data/rabbitmq"
    local mongodbDir="${NEW_DATA_DIR}/data/mongodb"
    local XRAY_USER="xray"
    ## FOR XRAY DEB/RPM change the ownership of RabbitMq to xray user
    if [[ "${INSTALLER}" == "${DEB_TYPE}" || "${INSTALLER}" == "${RPM_TYPE}" ]]; then
        changeOwnership_hook ${rabbitmqDir} ${XRAY_USER}
    elif [[ "${INSTALLER}" == "${COMPOSE_TYPE}" ]]; then
    ## FOR XRAY-COMPOSE change the ownership of RabbitMq to 999
    ## FOR XRAY-COMPOSE change the ownership of Mongodb to 999  
        changeOwnership_hook ${rabbitmqDir} ${RABBITMQ_USER}
        changeOwnership_hook ${mongodbDir} ${MONGO_USER}
    fi
   echo "";
}

# update erlang cookie value to System.yaml
updateErlangCookie ()  {
    local erlangCookieYamlpath=
    local erlangCookieFilePath=
    local sourceFile=
    bannerSubSection "Processing Migration of [.erlang.cookie] value to system.yaml"
    retrieveYamlValue "migration.erlangCookie.filePath" "erlangCookieFilePath" "Warning"
    erlangCookieFilePath="${YAML_VALUE}"
    if [[ -z "${erlangCookieFilePath}" ]]; then
        return
    fi
    retrieveYamlValue "migration.erlangCookie.yamlPath" "erlangCookieYamlpath" "Warning"
    erlangCookieYamlpath="${YAML_VALUE}"
    if [[ -z "${erlangCookieYamlpath}" ]]; then
        return
    fi
    local targetDataDir="`cd "${NEW_DATA_DIR}"/../;pwd`"
    sourceFile="${targetDataDir}/${erlangCookieFilePath}/.erlang.cookie"
    if [[ -f "${sourceFile}" ]]; then
        local erlangCookieValue="$(cat ${sourceFile})"
        setSystemValue "${erlangCookieYamlpath}" "${erlangCookieValue}" "${SYSTEM_YAML_PATH}"
        logger "Setting [${erlangCookieYamlpath}] with [${erlangCookieValue}] in system.yaml"
    else
        warn "Source file [${sourceFile}] does not exist in path"
    fi
}

# Check Xray product version
checkXrayVersion () {
    local minProductVersion="2.7.0"
    local maxProductVersion="3.0.0"
    if [[ "${INSTALLER}" ==  "${COMPOSE_TYPE}" ]]; then
        local newfilePath="${APP_DIR}/../.env"
        local oldfilePath="${OLD_DATA_DIR}/xray-installer/version.current"
    elif [[ "${INSTALLER}" ==  "${ZIP_TYPE}" ]]; then
        local newfilePath="${NEW_DATA_DIR}/../version-manifest.txt"
    else
        local newfilePath="/opt/jfrog/xray/version-manifest.txt"
        local oldfilePath="/opt/jfrog/xray/oldVersion.sh"
    fi

    getXrayVersion "${minProductVersion}" "${maxProductVersion}" "${newfilePath}" "${oldfilePath}"
}

removeVersionManifestFile () {
    if [[ "${INSTALLER}" ==  "${ZIP_TYPE}" ]]; then
        rm -f "${NEW_DATA_DIR}"/../version-manifest.txt || warn "Fail to remove ${NEW_DATA_DIR}/../version-manifest.txt"
    fi
}

readCustomDataDir () {
    # read custom data directory
    local setEnvFile="/opt/jfrog/xray/scripts/setenv.sh"
...
getCustomDataDir_hook () {
    ...
    if [[ -z "${XRAY_DATA}" ]]; then
        ...
        OLD_DATA_DIR="${XRAY_DATA}"
    fi
}

xrayInfoMessage () {
    addText "# yamlFile was generated from ha-node.properties and xray_config.yml files." "${SYSTEM_YAML_PATH}"
}
...
migrationXray () {
    ...
    checkXrayVersion
    _pauseExecution "checkXrayVersion"
    startMigration
    ...
    changeThirdpartyDirOwnership
    _pauseExecution "changeThirdpartyDirOwnership"
    updateErlangCookie
    _pauseExecution "updateErlangCookie"
    migratePropertiesFiles
    ...
    xrayInfoMessage
    _pauseExecution "xrayInfoMessage"
    removeVersionManifestFile
    _pauseExecution "removeVersionManifestFile"
    endMigration
